...
`jieqi-nearby-dealt.json` and `terminal/jieqi-nearby-ended-early.json` are the scenarios of the game with no frozen start, so their configuration always names one — and what it names is a deal rather than a position anybody composed. The second files that game as it stands, which is where the History replay re-verifies the deal against the start it spells with nothing but the document to do it from: the wire session that held the fourth value died with the game. A build that treated a named start as Free Play's alone would refuse to create it; one that folded a spelled-out start into an empty member would answer the wrong thing about where it began; and one that wrote the document without the deal's provenance, or with a deal the start does not spell, would fail its golden and its validation in turn.

All four modes are covered on both sides too. `nearby-two-moves.json` is the round trip of a game two devices play — its local side must survive a close and a reopen although no byte of the archive carries it, and its Undo must be refused — and `terminal/nearby-resignation.json`, `terminal/nearby-agreed-draw.json` and `terminal/nearby-mutual-resignation.json` are the three endings the protocol's explicit ends reduce to. `online-two-moves.json` and `terminal/online-resignation.json` are the second networked mode on each side, and they name no golden: an online document differs from its nearby twin in the mode string alone, and [`../archive/`](../archive/README.md) states each rule once rather than twice.
...
| the deal derivation | the protocol's derivation against two stated seed-and-nonce vectors, the second of which draws a value that must be discarded — the cross-implementation anchor a second implementation of the deal is checked against, rather than a test of this one |
