...
**The network is in the repository**, at `core/assets/`, beside the variant configuration it is loaded with and already under the name the engine's variant-matching rule requires — so the script copies rather than renames, and nothing has to be told where the bytes are. It still verifies their byte length and SHA-256 against `pinned-inputs.json` before staging them: that is what catches an `MXQ_NNUE_SOURCE` override pointed at the wrong bytes, and a damaged four-megabyte binary has no other symptom. Absence or a mismatch stops the script rather than producing an app whose AI is quietly a different opponent. The app's `Check the shared core is current` phase refuses a build whose network has not been staged, for the same reason.
