...
- **The Windows zip does not contain the AI's network file.** The application's other builds bundle it; this one cannot, because the artifact a CI build publishes on a public repository is downloadable by any logged-in account, and the network's redistribution licence has never been established — a gate [engine-integration.md](engine-integration.md) sets for distribution beyond internal testing. The zip carries instructions naming the file and where it goes, and the recipient adds it. Until they do, the application runs and every feature except the AI works.
