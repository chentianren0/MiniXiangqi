...
The piece symbols and the notation are two independent preferences rather than one: a player learning the characters may want icons on the discs while reading the Chinese move list they are learning from, and an international player wants both changed. Neither follows the interface language, because a device-dependent default surprises anyone comparing two machines and the accepted defaults are the same everywhere.
