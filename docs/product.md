...
- Distribution is internal only: TestFlight internal testing on Apple platforms and, on Windows, a zip built by CI that somebody unpacks and runs — one per supported architecture, needing no installer and no runtime install (owner decision, 2026-07-30; MSIX and its signing-certificate story are the post-MVP upgrade, complexity without proportionate gain at internal-education scale). There is no public App Store, Microsoft Store, or other public release plan.
...
- Decide whether the Windows distribution should become an MSIX package, and with it a signing certificate, once the zip has been in internal testers' hands. The mechanism question this entry used to ask is answered above — a CI-built zip per architecture — and the minimum tested configuration left it before that: the floor is Windows 11 on `x64` and `ARM64`, above.
