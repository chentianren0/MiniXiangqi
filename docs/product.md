...
- **Windows ships through the Microsoft Store, and the zip stays.** *(Owner decision, 2026-07-31.)* This bullet used to say that Windows distribution was internal only and that MSIX was a post-MVP upgrade waiting on a signing certificate. The premise was false: a package submitted to the Store is signed **by the Store**, with Microsoft's certificate, after it is accepted — so a Store submission never needs a certificate of ours, and the thing that was supposed to make MSIX expensive does not exist. The Store is therefore the intended public channel for Windows. The CI-built zip per architecture remains beside it as the direct download — unpack and run, no installer, no runtime install — because it is the channel that needs no account and no store, and because it is what internal testers already have. On Apple platforms distribution is still TestFlight internal testing, and there is no public App Store plan.
- **Every build contains the AI's network file, the Windows zip included.** *(2026-07-31.)* This bullet used to record an exception: the zip omitted the network, because a CI artifact on a public repository is downloadable by any logged-in account and the network then bundled had no established redistribution licence. The application now plays with a network this project trained itself, so there is nothing it cannot publish and no file a recipient has to add. What the AI plays like changed with it: the bundled network is measurably stronger than no network and measurably weaker than the one it replaced, which [engine-integration.md](engine-integration.md) states with the numbers.
...
- Windows 11 on `x64` and `ARM64`. Windows 10 is not a target: it left Microsoft support in October 2025, and a floor the vendor no longer patches is not a floor this product can promise (owner decision, 2026-07-30, narrowing what this document previously promised). `ARM64` came off the list the same day on the rule that an architecture nobody has run is a claim rather than a target, and returned on 2026-07-31 under that same rule rather than against it: the owner has ARM64 Windows hardware to test on, the engine and the core suites are built and run natively on an ARM64 machine in CI, and the frontend is built, exercised and distributed there. It is a target because it has been run, not because it is expected to work.
...
- Settle the app's Store name. **Xiangqi Master** is the owner's provisional choice (2026-07-31) and is what the package manifest carries; the name is finally fixed by the Partner Center reservation, which is also where the package identity comes from, so both settle in one act. Nothing else in the build changes when they land.
