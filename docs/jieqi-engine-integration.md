...
The change is required rather than convenient. The end-reason vocabularies in [core-interface.md](core-interface.md) and [game-data.md](game-data.md) name the rule that ended a game, and the unpatched function reports only that an ending fired and what it is worth: one draw value stands for a threefold repetition, a mutual perpetual check, a repetition that is no chase, and the forty-move rule alike. The Fairy-Stockfish fork carries a change of the same purpose for the other two xiangqi games, and the parallel is that purpose alone — the two touch different things and neither is a model for the other. Nothing else is asked of this fork.
