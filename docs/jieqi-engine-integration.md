...
The pinned branch carries focused changes, recorded in the manifest:
...
- **A chase renews per chasing piece rather than per chased piece.** The detector diffed the set of *chased pieces* across a move, so a target already under attack was dropped however it was attacked afterwards, and a chase whose attacker changed read as no chase at all — the shape that produces it being two chariots bearing on one target with a horse between them, each of whose moves blocks one line while it uncovers the other, so the target is never for a moment unattacked and no move ever attacks a piece that was not attacked before. The diff is now taken per chasing-chased pair: a piece is still chased when a piece attacks it that was not attacking it before the move. That is [jieqi-rules.md](jieqi-rules.md)'s adopted renewal reading followed per attacker, and a chase handed from one attacking piece to another is one the detector now keeps. What the change carries is that reading and nothing further: it is not a claim that this engine's chase adjudication agrees with the contract in every shape, and which shapes are pinned is the `jq-` fixtures' answer rather than this document's.

Every change is required rather than convenient. The first: the end-reason vocabularies in [core-interface.md](core-interface.md) and [game-data.md](game-data.md) name the rule that ended a game, and the unpatched function reports only that an ending fired and what it is worth — one draw value stands for a threefold repetition, a mutual perpetual check, a repetition that is no chase, and the forty-move rule alike. The second: no validation on this side can prevent the overrun, because the position that causes it is a legal one that the game reaches by playing, and a core that refused it would be refusing the game. The third: the rule it reads is one this project owns and the fixtures state, the reading it replaces returns a claimable draw where the contract names a loss, and nothing above this bridge could recover that — chase detection is the engine's alone, and a core that second-guessed it would be the second reading of the rules this architecture exists to forbid. The Fairy-Stockfish fork carries a change of the first's purpose for the other xiangqi games, and the parallel is that purpose alone — the two touch different things and neither is a model for the other. Nothing else is asked of this fork.

## The rules slice

- **Only `position.cpp`, `movegen.cpp` and `bitboard.cpp` compile.** Their include closure is much wider than they are, carrying header declarations from the engine's search, UCI and network layers, and **not one of those layers is compiled**: the core builds those objects, and a declaration nothing calls costs nothing. One network header contributes a constant table the position code reads, and no evaluation, search, thread or option code enters the core beside it.
- **The symbols the closure leaves undefined are supplied as one-line stubs by a link closure of the slice's own**, a file beside the vendored sources rather than part of the bridge: a prefetch hint, the transposition table's first-entry accessor, and the square-name formatter. None of them has anything to do — the prefetch is advisory, the table accessor is unreachable because the bridge hands the position no table, and the formatter serves only a stream operator nothing calls — and each is declared in the engine's own namespace, which the rename below reaches. Inert stubs and a wide closure are together the price of leaving the vendored sources exactly as the fork wrote them, and that is lower than the price of a source change to be carried, rebased and re-reviewed at every pin.
...
- The snapshot carries those sources and their whole include closure, because that is what the unedited sources ask for. Only those compile; the rest arrive as declarations, one of them a network header contributing the constant table the position code reads, and nothing in the snapshot loads a network.
...
- **The build is netless, and no network exists to make it otherwise.** This engine's feature set is not the one published Pikafish networks are trained for, and the engine rejects such a file by its own hash check. Nothing is shipped, staged or verified: this engine has no asset of any kind, and the weight preflight the other engines run has no counterpart here.
...
Named because a reader of the other engine contracts will look for them:
...
`pinned-inputs.json` is the single source of truth for this engine's inputs as it is for the others, and records them separately from each:
