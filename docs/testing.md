...
- Verify the accepted board-metric rules at the smallest supported size and at a large one: no piece style draws decoration beyond `0.40 p`, no marker's ink falls inside `0.42 p`, and no marker leaves its own `1 p` cell at rest or at any moment of its animation, including the selection lift, a drag's target strengthening, and the check pulse.
- Verify each style's marker ink meets both accepted strengths against that style's own board surface — active at 4.5:1 and record at 3:1 — in light and dark appearance, and that record ink is promoted to active values under Increase Contrast.
- Verify the board renders identically with Differentiate Without Color enabled and disabled.
...
- Verify the **将军** token is present for exactly as long as the side to move is in check, that the check rings hide while a checked general is selected or dragged, and that they return when it is released, including after an abandoned drag.
- Verify last-move brackets follow the game's current last move through Undo rather than remaining on the discarded move, and that no brackets appear at an initial position.
