...
The UI bundle carries two sets of suites that are not ports of each other, and the platform is declared **per file** rather than by the target. The macOS suites — `PlayScreenUITests`, `PlayHomeUITests`, `HistoryScreenUITests`, `SettingsScreenUITests`, `HumanVersusAIUITests` — are `#if os(macOS)`: they drive a window, naming a size and reading the frame back, and a window is what iOS has not got. The phone suites — `PhonePlayUITests`, `PhoneSettingsUITests` — are `#if os(iOS)`, and they ask the questions only a phone can answer: that a 402-point phone takes the **stacked** arrangement, that Mini Xiangqi holds its 44-point pitch floor and Xiangqi its 34-point floor, that the 49 or 90 hit cells are real and non-overlapping, that two taps make a move, that the **Moves** record comes up over the board on demand and goes away again, that portrait remains fixed, that the result flow reaches a filed record, and that the **Haptics** row agrees with the hardware. A destination therefore selects its own suites, and `LaunchPreferences` keeps the hermetic-launch table single rather than letting two copies drift.
...
- Verify no captured-piece display appears in either layout shape, in any game whose position is wholly public.
- Verify a jieqi hidden loss stands as face-down discs at its row's end — the face-down body, never a stone — and resolves into identified pieces at disclosure. Verify a shown loss that left the board face down is told from an open capture, its symbol partway arrived, and that the telling survives disclosure. Verify the resident section stands at the concealing board's floor pitch and extends with its rows without scrolling, and that the reached sheet draws its discs at the board's own width-fitted pitch.
- Verify the play controls are **Undo · Claim Draw · Resign · Flip Board** in human-versus-AI, **Undo · Claim Draw · Flip Board** in Free Play, and the transport plus **Flip Board** in replay; and that a finished game carries **Undo** · the concluding action · **Flip Board** in both modes. What bounds the play cluster is judgement rather than a count — small and calm, neither crowding the screen nor reading strangely — and it is judged from a rendered screen; replay's transport is not part of it. Verify the flip is presentation only in every mode: it changes no game state, takes no tint, and is never disabled.
- Verify **Resign** presents **Resign?** / **Resigning records this game as your loss.** with **Cancel** and **Resign**; that confirming records a human loss and an immutable History record; that cancelling changes nothing; and that it is absent in Free Play and replay.
...
- Verify that a new installation defaults to **I Move First** and **Standard**, while Settings can persist **AI Moves First**, **Random**, and any accepted AI level as later setup defaults.
...
- Verify that the setup board is a noninteractive preview, no active game or resolved Random side exists before **Start Game**, and the created game freezes the resolved side, level identifier, and exact `movetime`. Random previews Red at the bottom and flips only after successful creation resolves to **AI Moves First**.
- Verify the Play root has no board and offers four rows in two headed sections, in order: **Xiangqi** then **Mini Xiangqi**, each **Human versus AI** then **Free Play**. Every row opens setup carrying that exact game. With an active game, the **Current Game** metadata begins with its game and **Resume Game** restores it exactly; a fresh launch lands on this page rather than in the game, leaving ends nothing, and a filed game is not current.
...
- Verify the exact fixed copy: **Start a new game?**, metadata header **Current Game**, **This game will be saved to History as it is now.**, **Cancel**, and **Save and Continue**. Verify that only metadata changes and that no separate Undo or draw-claim action appears in this confirmation.
...
- Verify that both modes of both games open a pre-start state, visibly name the selected game, and create no active game before **Start Game**. Free Play uses a noninteractive, Red-bottom preview with no turn status, shows **You control both Red and Black. Red moves first.**, and presents no configurable setup fields.
- After successful **Save and Continue**, verify that leaving either pre-start state creates no game and the old game remains immutable History.
- Verify that a failed archive-and-clear operation presents **Couldn't Save the Game**, **The current game is unchanged. Please try again.**, **Cancel**, and **Try Again**; preserves the old active game; creates no new game; and never enters a pre-start state. Verify Retry repeats the atomic operation and Cancel discards the temporary destination.
...
- Test double **Start Game**, leaving while creation is in progress, and late completion for both modes. Only one current setup-session request may commit; leaving invalidates it and creates no game.
...
- Verify the natural-result notice before and after saving, including **Save**, **Save and New Game**, **Replay**, **Done**, the absence of **Undo** on the notice and its presence in the cluster, and the absence of a Play Again action. Verify it is dismissible by its own close control, by the cancel key, and by a click on the board; that closing it decides nothing, leaving the result on the turn status, Undo available while the result is unconfirmed, and the concluding action in the cluster; and that it does not present itself again for the same result. Verify that a game already filed is not filed again by **Done** or by the cluster's **New Game**.
- Verify the threefold notice, **Keep Playing**, **End as a Draw**, and retained non-blocking **Draw Available** affordance in both play modes.
...
- Verify **Confirm Before Deleting** defaults on and governs every Delete entry point — the visible action, the complete swipe, the context menu, the keyboard, and the screen-reader custom action. Test the accepted confirmation copy, Cancel, confirmed deletion, immediate deletion when disabled, persistence failure, and the absence of deletion Undo or Recently Deleted.
...
- Verify that undo persists only the retained main line, provides no redo, and remains available after a natural result only until result confirmation or successful **Save and Continue**.
...
- Verify **Fast**, **Standard**, and **Deep** use `go movetime 1000`, `go movetime 3000`, and `go movetime 5000`, respectively, and that **Standard** is the new-install default.
...
- Verify that an allocation failure at a budget of at least 256 MiB presents the accepted **Couldn't Start the AI Opponent** notice unchanged, that Retry re-probes and recalculates, and that no smaller Hash is substituted.
...
- Verify Reduce Motion replaces position, scale and rotation with a crossfade while leaving opacity, colour, stroke and shadow animations, ordering, and every state intact, and confirm specifically that the check pulse is removed while the check rings, the **Check** token, and the illegal-tap response all still deliver their states without animation.
...
- Verify the per-ply save-failure capsule shows **Couldn't save that move. Please try again.** for a failed user move or Undo only, that no capsule appears when the failed save is the AI's reply, and that a failed draw claim, resignation, or result confirmation uses the **Couldn't Save the Game** presentation instead.
...
- Verify the numeral strips follow the notation preference and game: both edges Arabic with WXF selected, Red's Chinese with **Chinese** selected, each side numbering from its own right, seven entries for Mini Xiangqi and nine for Xiangqi, with equal visual weight.
...
- verified GPLv3 and license inputs, carried in the artifact where the artifact is one somebody is handed: the Windows zip contains the project's own `LICENSE` and a notice naming Fairy-Stockfish with the pinned fork revision, SQLite's public-domain dedication, the Microsoft redistributables it carries, the project-trained Mini Xiangqi network with its reproducible pipeline, and the redistributed GPL Xiangqi network with its source. Each network's filename, byte length, SHA-256 and provenance come from the manifest. **The Store package is a distribution under this same clause and carries the same two documents**, generated by the same script from the same manifest; only the sentence about which Microsoft runtime is installed rather than carried differs between them, and "the Microsoft redistributables it carries" is read against what the package actually holds;
- **both bundled networks' verified pinned hashes, in every distribution without exception**, so that an absent network is damage rather than a case a recipient has instructions for;
