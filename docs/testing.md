...
- **The Apple simulator pair this project takes its iOS evidence on is a current iPhone — iPhone 17 or iPhone 17 Pro — and a current iPad — iPad Air or iPad Pro.** *(Owner, 2026-07-30.)* The phone gives the compact-width evidence and the iPad the regular-width evidence in both orientations, which is every arrangement the layout has. Older or smaller devices are not part of it. **One simulator is booted at a time**: several at once is enough to put a development Mac under memory pressure, which is a poor way to measure an app about memory.
...
- Verify both the layout shape and the navigation presentation are selected by the available space rather than by device identity, so a resized macOS window and a windowed iPad reach the same arrangement at the same size, and that one navigation container serves all three platforms.
