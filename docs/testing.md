...
- Verify both numeral strips are present, that each faces the player whose numerals it shows, that they follow the board's orientation, that Chinese and Arabic sit on a shared baseline, that the numerals are set semibold and the digits bold, and that every numeral reaches 4.5:1 against the board surface and 7:1 under Increase Contrast.
...
- Verify move travel takes the accepted duration at each of the seven board distances, that a decision-cycle Undo completes within 600 ms, and that board flipping takes its accepted duration.
...
- Verify Reduce Motion replaces position, scale and rotation with a crossfade while leaving opacity, colour, stroke and shadow animations, ordering, and every state intact, and confirm specifically that the check pulse survives it.
- Verify sound and haptics fire when the event completes rather than when its animation begins, except for selection feedback, which leads its animation.
- Verify exactly three custom glass surfaces exist, that exactly one is present during ordinary play and system-provided glass is not counted among the three, that no glass intersects the board block including its margin and numeral strips, and that the AI-thinking indicator carries no material.
