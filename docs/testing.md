...
- `.github/workflows/core-suites.yml` is that CI: the core suites in both configurations, on a pinned macOS runner and a pinned Windows runner. It runs whatever toolchain those images carry, which is not the same claim as reproducing the pin — on 2026-07-30 the `windows-2025` image happened to carry the same MSVC toolset the pin names, from a different Visual Studio edition, and the `macos-15` image carries an older Apple clang than the developer machines do. What CI proves is that the core compiles and passes on more than one machine; reproducing the pinned build is a developer-machine claim.
