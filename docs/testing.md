...
- Verify adjudication does not depend on side-to-move parity: enter one repeating sequence by a quiet move from each side in turn, and confirm the same verdict at the same occurrence, including that a mutual perpetual chase is a draw rather than a loss for whichever side entered it.
