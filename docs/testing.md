...
- Verify the natural-result notice before and after confirmation, including **悔棋**, **结束对局**, **回放**, **完成**, and the absence of a Play Again action. Verify it is dismissible by its own close control, by the cancel key, and by a click on the board; that closing it decides nothing, leaving the result on the turn status and the concluding action in the cluster; and that it does not present itself again for the same result.
