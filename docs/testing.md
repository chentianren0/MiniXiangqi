...
- Verify the Play root has no board and offers four rows in two headed sections, in order: **象棋** then **迷你象棋**, each **人机对弈** then **自由对弈**. Every row opens setup carrying that exact game. With an active game, the **当前对局** metadata begins with its game and **回到对局** restores it exactly; a fresh launch lands on this page rather than in the game, leaving ends nothing, and a filed game is not current.
...
- Test each answer a version other than the defined one gets, per [game-data.md](game-data.md), and that nothing is migrated on either axis: an archive of any other version is refused and the file is left alone; a store recording a newer schema is refused and left alone; a store recording a schema below the defined one is replaced by a fresh, empty library, which a record that was in the file and is not in the library afterwards is what proves; and a store that cannot be positively identified is refused rather than replaced. A test that named a particular older shape would be the only thing in the repository that did, so each states the comparison instead.
