...
| `control.import` *(proposed)* | 导入… | Import… | toolbar item | [interaction-design.md](interaction-design.md) § History library; `History/HistoryScreen.swift` |
| `control.view` *(proposed)* | 查看 | View | alert button | same — the duplicate answer's way to the record it found |
| `control.ok` *(proposed)* | 好 | OK | alert button | same — the acknowledgement of an alert that is informational plus at most one navigation |
...
| `alert.importDuplicate.title` *(proposed)* | 这盘棋已经在历史里 | This Game Is Already in History | alert title | [interaction-design.md](interaction-design.md) § History library; [game-data.md](game-data.md) § Accepted MVP record behavior; `History/HistoryScreen.swift` |
| `alert.importDuplicate.message` *(proposed)* | 文件里的对局和历史中的一盘完全相同，所以没有重复添加。 | The game in this file is identical to one already in History, so it wasn't added again. | alert message | same |
| `alert.importConflict.title` *(proposed)* | 这个文件和历史中的一盘棋冲突 | This File Conflicts with a Game in History | alert title | same |
| `alert.importConflict.message` *(proposed)* | 它和历史中的一盘棋是同一局，但内容不同。历史没有改变。如果要用这个文件，请先删除历史中的那一盘。 | It is the same game as one in History, but its contents differ. History is unchanged. To use this file, delete that game from History first. | alert message | same |
| `alert.importNewerVersion.title` *(proposed)* | 这个文件由更新版本的 Mini Xiangqi 创建 | This File Was Created by a Newer Version of Mini Xiangqi | alert title | [game-data.md](game-data.md) § Migration — the one message the compatibility promise requires to be distinct |
| `alert.importNewerVersion.message` *(proposed)* | 当前版本无法读取它。请更新 Mini Xiangqi 后再试。历史没有改变。 | This version can't read it. Update Mini Xiangqi and try again. History is unchanged. | alert message | same |
| `alert.importUnreadable.title` *(proposed)* | 无法读取这个对局文件 | Can't Read This Game File | alert title | [interaction-design.md](interaction-design.md) § History library |
| `alert.importUnreadable.message` *(proposed)* | 文件的内容无效或过大，无法导入。历史没有改变。请确认文件完整，或者向对方要一份新的。 | The file's contents are invalid or too large to import. History is unchanged. Check that the file is complete, or ask for a new copy. | alert message | same |
| `alert.importSaveFailed.title` *(proposed)* | 无法保存导入的对局 | Couldn't Save the Imported Game | alert title | same |
| `alert.importSaveFailed.message` *(proposed)* | 对局文件没有问题，但保存到历史时出错。历史没有改变。请重试。 | The game file is fine, but saving it to History failed. History is unchanged. Please try again. | alert message | same |
...
| `metadata.imported` *(proposed)* | 导入 | Imported | row prefix on an imported record | [interaction-design.md](interaction-design.md) § History library; `History/RecordMetadata.swift` |
...
- Approve the fourteen *(proposed)* rows interchange added: **导入…**, **查看**, **好**, the imported marker **导入**, and the five import answers as title-and-message pairs. Four of them are the designed answers to a rejection and one is the answer to a duplicate; between them they are every sentence an import can say other than the row appearing. Two notes a reviewer will want. **好** is used where HIG would prefer a more specific word: nothing is being cancelled, and 完成 and 关闭 both imply a task ended, so 好 is the least wrong of the available titles for an alert that is informational plus at most one navigation. And the rejection set is four rather than the Part 7 draft's five: that draft separated *this is not our file* from *this file cannot be read*, and the core's taxonomy does not — both are the same status, and telling them apart above the interface would mean branching on a diagnostic string the contract reserves for logs. The picker is where the wrong-file case is actually prevented.
