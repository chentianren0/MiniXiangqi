...
Every board dimension is a multiple of the **cell pitch** `p`, the distance between two adjacent points, so the board scales from its smallest supported size to a large window without any dimension being re-tuned and without the relationships between them changing. The accepted floor is `p ≥ 44 pt` on every interactive board, fixed under [Layout shapes](#layout-shapes) below. A pre-start preview is not interactive and carries no floor — with one recorded platform exception, described with the exemption itself under [Layout shapes](#layout-shapes).
...
**On Windows the edges carry coordinates instead, for as long as the record does.** The Windows move record is the core's own canonical coordinate text — `d1d3` — and the strips exist so a player can map the record to the board, so a 一二三 edge beside that record would map to nothing a reader could follow. Windows therefore labels all four edges with the coordinates the record itself uses: files **a**–**g** above and below the board core, ranks **1**–**7** to its left and right. Ranks are labelled there and nowhere else, because the canonical notation names a rank and traditional notation never does. A coordinate is absolute, so flipping the board re-orders the labels and never renames them, and neither edge belongs to a player — the rule that the numbering beside a player is that player's own has nothing to be about here. The board block no resident surface may intersect is correspondingly the core together with all four strips, which is square. It supersedes [WXF rendering](#wxf-rendering)'s numeral-strip clause on that platform for the same span and for the same reason — that clause makes the strips follow the **记谱法** preference, and Windows has neither the preference nor a rendering for it to select. When the preference and the two proper renderings arrive there, both clauses resume and these coordinates go with the record that needed them.
...
**The Play destination's root is an independent page for choosing what to play, and there is no board anywhere on it.** What the player is doing on this page is choosing what to play; a preview of a board they have not chosen yet answers no question they are asking, and a page that is a chooser rather than a board is the one that can hold a second game later.
...
- **Where the destination bar stands across the bottom, this screen hides it**, which is the general rule in [Navigation](#navigation). What the reclaimed height buys is the **board**: on a 402-point iPhone the pitch reaches 50, the largest that width allows, and the move list keeps four whole rows. The list gains no fifth row because there is none left to gain — the board stops at the width and the remainder is air.
...
The rule's outcomes at the accepted sizes: on an iPad Pro 11-inch in portrait the stacked board reaches the 102-point maximum where the panel beside it would have held the board to 75; the same device in landscape goes side by side at 88, where the height is what bounds the board and the panel costs it nothing; on the smallest Mac window the stacked shape cannot reach the 44-point floor at all, so side by side stands there; and **a Mac window that is tall and narrow stacks** — the first such window is 616 by 535 points of content, and a half-screen tile on a laptop, where the app's own minimum holds the width at 616 while the height runs to the full screen, is the everyday case, with the stacked board close to twice the pitch the panel beside it would leave. Near the boundary the win is as small as one point of pitch, paid for with the resident move list; that is the rule's weakest moment, and it is pinned by a test at the exact size the answer changes rather than smoothed over with a second number.
...
That floor is affordable on the most constrained configuration measured: a built-in Retina display running at 1024 by 663 points — the largest-text setting on a current Mac — leaves a window of 1024 by 582 points once the menu bar and the Dock are subtracted, and 550 points of content height below a standard title bar. At the 44-point floor the board core is 308 points square. Whether every accepted state fits that budget on every supported device is a question for a running frontend rather than for arithmetic in prose.
...
**The play content's floor is 616 by 388 points**, which is the sum of what the two floors ask for and nothing else: the board block at its 44-point floor is 308 by 340, the air around it is 24 points a side, and the panel beside it is 260. A window one step narrower clips the board against the panel, and one step shorter carries it up behind the title bar.

**macOS stops at 760 by 492 points**, which is that floor plus the navigation container: 144 points of sidebar beside it and 52 of toolbar above it, both measured on the running app rather than derived. The 616 by 388 the play content gets inside that window is unchanged, which is the point — the destination structure is added around the layout rather than out of it. The number moves if the container's presentation changes.
...
**Windows stops at 696 by 432 points**, over a play content floor of **648 by 388**. The 648 is the 616 above recomputed for a board block 340 wide rather than 308 — that frontend's move record is the core's canonical coordinate text, so its board carries the coordinates on four edges instead of the file numerals on two, and the block comes out square. Around that content go the two pieces of chrome it has: the shell's 48-point compact rail beside it, and the 44-point navigation row every page shares above it. Every figure is derived in `windows/MiniXiangqi.Play/Board/BoardSpace.cs` from the board's own geometry rather than written down, so a geometry change moves the floor with it. **The container's own pane cannot squeeze the board below it**: the platform shows a full-width inline pane only where the window is wide enough to pay for one, and an opened pane below that width overlays the content rather than taking any of it — so the floor pays for the rail, which is the arrangement that actually costs it width.
