...
**On Windows the switch is not offered at all, and the group is 声音 alone.** *(Stage 6, Windows.)* This is the third answer the hardware question has, and it is the same answer iOS gives on a device with no engine — the row is **removed rather than greyed out** — arrived at from the opposite direction. On iOS the question is asked of the device because the answer varies between devices; on Windows there is nothing to ask, because the platform offers an application no way to drive the machine it is running on. `Windows.Devices.Haptics` addresses pens and game controllers — accessories the app has no dealings with — and there is no counterpart to the Mac's system haptic performer for the machine's own trackpad. A toggle over that would be a control that never does anything on any Windows machine, which is exactly what "unavailable rather than silently ineffective" forbids; the group survives losing the row for the reasons already given, having no header to strand and no dependency between its two switches. The key is not written either: `haptics.enabled` is left exactly as an Apple frontend stored it, since a Windows launch has nothing to say about hardware it cannot see.
...
- Define the haptic events outside board play. Board play's haptic set is defined under [Sound and haptics](#sound-and-haptics), and each half now follows its own accepted preference wherever it would fire. Where and how the two toggles are presented is settled there too, on all three platforms: Windows gave its answer to the hardware question at Stage 6 and offers 声音 alone.
