...
- **Last move.** Four L-shaped corner brackets on both the origin cell and the destination cell: arm `0.16 p`, stroke `0.045 p`, inset `0.05 p` from each cell corner, record ink. The inset is what keeps two adjacent cells' brackets visibly separate, which matters because a one-step move puts them side by side. Angular where every other marker is circular, so they are unmistakable at any size. They always mark the move that produced the position on screen: an Undo moves them to the move that is now last rather than leaving them behind, replay moves them as it steps, and no brackets are shown at an initial position. The AI's move uses these same brackets rather than a second marker — there is only ever one last move, and whose it was is carried by the turn status and the accessibility announcement.
