...
- **Pointer hover — on Windows.** The point under the pointer takes a faint rounded-square fill in marker ink at low opacity, drawn beneath the pieces. It reports where the pointer is, not what is legal. A hover never previews a piece's legal destinations: selection already answers that question in one click, and a preview offered before any selection sits at the edge of the hints and analysis the product excludes. **The fill belongs to Windows alone.** A faint fill under the pointer is the Fluent idiom's own answer to where a pointer is, and the Mac's board deliberately stays quiet. The layering order below and the marker-ink measurement above still name the fill, and name it where it exists.
