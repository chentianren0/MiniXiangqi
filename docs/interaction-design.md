...
- **That pair is the top of the page in both layout shapes.** Beside the board it is the panel's first section, which is already the top. Beneath the board it is **above** the board rather than under it — where the stacked arrangement puts what is true of the game, and where the play screen puts its own turn status — and the panel beneath the board keeps the height it already asked for, so the whole of the pair's former slot inside it goes to the move list. The screen is therefore one design in both shapes: what the game was, then the board, then the record and the transport. **The board pays for the move wherever the shape is bound by its height**, and that cost is accepted rather than hidden: on a 402-point iPhone the board comes to rest exactly on the 44-point floor, which replay can afford because its board is a read-only document with no touch target on it, and on an iPad in portrait the pitch goes from 98 to 85. Where the space is shorter still it is the panel that yields, not the board — the grant under [Layout shapes](#layout-shapes) reserves the board's floor first. *(Owner feedback from the iPhone, 2026-07-30: the result line moves to the top of the page and the freed bottom space goes to the move list.)*
