...
- The claim is the player's to invoke. **Claim Draw** presents the blocking notice — title **This position has occurred three times.**, message **You can end the game as a draw.** — which offers **Keep Playing** and **End as a Draw**.
...
The supported languages are Simplified Chinese and English. Simplified Chinese is the source language: the accepted Simplified Chinese is normative, and its English counterparts are translations of it.

Every string the interface shows lives in the shipped string catalogs — `Localizable.xcstrings`, and `InfoPlist.xcstrings` for the one sentence the system reads out of the bundle — under a symbolic key rather than under its own source text, in both languages. They are the string of record: the strings quoted throughout this document describe behavior, and where a quotation and a catalog entry disagree, the entry is the copy.
