...
- Define the sound events outside board play — whether the History and Help surfaces sound at all; Settings is settled under [Sound and haptics](#sound-and-haptics) and is silent — and the per-platform differences behind the accepted sound toggle, including what an iOS device does with a board sound under the silent switch. Board play's four sounds, their precedence, their character, and the events that stay silent are defined under [Sound and haptics](#sound-and-haptics), and their samples are tuned by ear against the running app.
