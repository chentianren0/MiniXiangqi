...
- **A game is a headed section, and its ways to play are the rows in it.** The sections appear in this order: **象棋** then **迷你象棋**. Each offers **人机对弈** then **自由对弈**. Selecting a row opens the pre-start state for exactly that game and mode.
...
- In human-versus-AI play, the human player's side is displayed at the bottom. Choosing **我先手** previews and resolves Red; choosing **AI 先手** previews and resolves Black; choosing **随机** resolves the human side only when the game is created and then applies the corresponding orientation. **That is where the board starts and not where it must stay**: once the game is under way the visible **翻转棋盘** described under [Play controls](#play-controls) turns it over, and a new game starts the right way up again. The pre-start state offers no flip control — there is no game yet to look at from the other side.
...
- **Nearby** — the acts of whoever's turn it is, then **认输** and **翻转棋盘**, per [Nearby play](#nearby-play). Never more than four, which is the composition this list's fourth-control rule already answers for.
...
- **Stacked.** The cluster carries it, in one row. At 402 points the row holds four controls with room to spare in Chinese, and holds them in English with **翻转棋盘** at its symbol, which is the fallback the cluster already had.
- **Side by side.** The panel is 260 points wide on both platforms and the cluster has 228 inside its inset, but the two frontends spend that differently. Windows stacks its play controls in a column of their own width, so a fourth costs height and the panel simply carries it. The Apple frontend lays them in a row, so a fourth costs width — and four English labels come to 359 points, more than the row has even with the trailing word given up. There **翻转棋盘 drops to a second row beneath the others**, which is what the Apple replay cluster already does with this same control in this same panel, and it keeps its word where the row above had no room for it. Chinese fits the row at the symbol and stays in it.
...
While a game is finished there is no draw left to judge, so that slot carries the concluding action instead, and **认输** goes with the game it could have ended: the finished cluster is **悔棋**, the concluding action, and **翻转棋盘**, in both modes, for as long as the game is finished.
...
**Mid-game, the same situation keeps its name and gains a guarantee.** When the app returns from suspension owing a search and preparation fails, the alert keeps the title **无法启动 AI 对手** — the situation is the same one, memory is not available right now, and a second title would name a distinction the reader cannot act on — but its message adds the one thing the pre-start case has no need of: **当前可用内存不足。对局已保存，可以稍后继续。请尝试关闭一些其他 App，然后重试。** Its actions are **稍后** and **重试**, not **取消** and **重试**, because there is nothing to cancel. After **稍后** the stalled state lives where things about the game live — the turn status's AI activity slot, with an inline **重试** — and every retry re-probes fresh. Undo remains available throughout and is itself a way out: removing the human's last move returns the game to their decision point, where no search is owed and the stalled state has nothing left to be about.

### Saving the active game before choosing a new mode

The [Play home](#the-play-home) shows the active game's metadata under a **当前对局** header and a direct **回到对局** action beside it. The metadata begins with the game and mode, then identifies the human's side when applicable and the move count. It shows the side to move for an ongoing game, the result and reason for a terminal game, and claim availability when applicable. The three state classes are exclusive and each takes one shape: an ongoing game is **进行中** and whose turn it is; a claimable repetition is **进行中** and **可判和**, the standing offer taking the side-to-move slot; a terminal game is its result and the reason for it, in the same longer register the History row uses.
...
Only the metadata changes; the title, message, and actions do not interpolate game-, mode-, or state-specific wording. Example metadata lines include **象棋 · 人机对弈 · 你执红 · 进行中 · 轮到黑方 · 42 步** and **迷你象棋 · 自由对弈 · 红方获胜 · 将死 · 42 步**. The metadata reports facts and does not ask the user to classify the result.
...
- On macOS the two actions sit side by side, with the default one trailing, where this platform puts a default action. **iOS keeps them side by side too**: at iPhone width, **保存并开始新对局** and the longer English *Save and New Game* both sit beside **保存** without crowding the notice or reaching the screen's edges. Permission to stack them stands for a size that has not been met.
...
- **以和棋结束** confirms an immutable draw record in History, and presents the claimed draw through the result notice without **悔棋**: the claim is the player's own confirmed act, and it is the one finish that cannot be walked back.
...
- **The entry is a third row inside each game's section on the [Play home](#the-play-home)**, **附近对弈**, under that game's **人机对弈** and **自由对弈**. It exists on iPhone and iPad and never on macOS. The row stands wherever either way of reaching another device could carry a game, which on those two is always; where none could, it is **absent rather than disabled**, a row that can never be pressed being a promise the hardware cannot keep, and no explanation helping a reader who cannot change the answer. The row opens the nearby game already going in that game, exactly as the **当前对局** card opens a local one; with none it raises that game's propose sheet.
...
- **A refusal is presented by its reason.** Every reason the protocol carries, and every refusal this device's own engine makes, has a sentence of its own in [copy.md](copy.md). No wire code and no diagnostic reaches the reader. Its title says which of the two it is — a game that never began, or one that was under way and is not any more.
- **The nearby board is the play screen's board**: the same two [layout shapes](#layout-shapes), the same [turn status](#turn-status), the same [markers](#game-state-markers), the same motion and the same sounds. The board accepts a tap only while the session is bound to a connection and the ply is this device's; off-turn it is quiet and unmarked, and a tap it cannot accept takes the acknowledgment beat like any other. **认输** and **翻转棋盘** are on it throughout, the way out replacing **认输** once the game is over.
- **The negotiations belong to whose turn it is**, because the protocol's own do: an offer and a request are the off-turn player's to open, a claim is a turn action, and an acceptance answers something that arrived. Off turn the cluster carries **提和** and **悔棋** — the take-back being of that player's own last move — and on turn it carries **判和**, with **接受** in its place while the other player has something standing. Each is available exactly when the session engine allows it and disabled otherwise, which is the grammar the cluster already keeps for **判和**.
- **The claim is the local game's claim.** Where the engine says it stands for this device's player, the enabled **判和** control and the turn status's **可判和** line are the standing offer, and pressing it presents the same confirmation in the same words as [Claimable threefold repetition](#claimable-threefold-repetition). Claiming ends the game as the rules-decided draw, through the result notice like any other end.
- **What the other player asked for is a line, not a prompt.** **对方提和** or **对方请求悔棋** stands in the turn status's own quiet register beside the **接受** that answers it, and nothing blocks the board: playing on is the other answer, and it needs no control, because a move voids what was standing and takes the line and the control with it. An accepted draw ends the game as **协议和棋**; an accepted take-back is shown with the board's own Undo motion.
...
- **A game that ends without a result says why, once.** The other device no longer holding it, the two devices ceasing to agree about it, and the other player starting a new one are the three ways it happens, and each is one sentence in that same notice, over **对局已结束**. The board stays where it is behind it — the position is still worth looking at — and nothing else is said: not the connection, not a second sentence, and never anything about what the app did next.
...
- **The row is two lines.** The first is when the game ended. The second is the accepted metadata composition applied to a filed game — `游戏 · 模式 · [执子] · 结果 · [结束原因] · 步数` — which is the save-and-continue confirmation's own vocabulary rather than a second one invented here. The mixed library remains one pinned-first chronological list: game identity is a metadata fact, not a filter, badge, or third grouping. The second line never truncates, so every required token wraps rather than disappearing.
...
- All but one of those messages say 历史没有改变 explicitly, because no persistent change is the guarantee the core makes and a reader has no other way to know it held. The exception is the damaged record, which is the one answer that is not about the file: its message names the damaged record and the deletion that is the way past it, and 历史没有改变 there would be a guarantee about a library the message has just called damaged. The strings themselves are [copy.md](copy.md)'s, and where a sentence here and a row there disagree the row wins, by that document's own rule.
...
**And under every floor, a board that cannot be drawn says so rather than showing nothing.** A minimum window is arithmetic, and arithmetic has a residue: a display scale rounded differently, a window frame the minimum is measured across, a container default that moves under a platform update. Where the space genuinely will not hold a board at the accepted pitch floor, no board is drawn — a board below the floor is a decision this design declines to make quietly — and the space it would have taken carries one line asking for a larger window, `board.tooSmall` in [copy.md](copy.md). It is not a failure screen and takes none of that family's treatment: nothing went wrong, nothing was lost, and one drag of a window edge puts it right.
