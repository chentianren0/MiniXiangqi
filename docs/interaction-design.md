...
**Where the fourth control goes belongs to the layout rather than to this list.** Side by side, the panel simply carries it, in the last place Free Play's cluster already carries it, and there is nothing to decide. The stacked layout is where the judgement above bites — the cluster is bounded by not crowding the screen and not reading strangely, and on a phone three controls is already what that allows — so the placement there is settled against a rendered screen like every other question this section leaves open, and issue #80 carries the answer per platform.
