...
- When more than one of the mover's files carries two or more of the same type — reachable only for soldiers, as 2-2 or 3-2 — the leading word alone no longer says which piece moved, so the origin file returns after the piece name: 前兵六进一, 后卒3进1. Pieces alone on their file keep the plain form, and this can never combine with the numbered form, which needs four on a single file. The forms beyond the rulebook's own 前, 中 and 后 are this contract's reading of established practice rather than a transcription of a normative source — the sport's rulebook stops at three — and are recorded as such, the way [xiangqi-rules.md](xiangqi-rules.md) records its adjudication readings.
