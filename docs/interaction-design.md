...
- Three of a type on one file take 前, 中, and 后; four or more are numbered from the front — 一兵, 二兵, and so on, in the mover's own numerals. Both open the move and omit the file exactly as the two-piece form does, and both are standard Xiangqi notation rather than an invention of this contract. Neither is a curiosity here: five sideways-capable soldiers a side make each reachable.
