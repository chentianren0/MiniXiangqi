...
**The two board screens hide the destination bar where that bar stands across the bottom of the screen.** The board and [History replay](#history-replay) hide it while they are up; the three homes — the Play home, the History list, Settings — keep it, and so does a mode's pre-start page. Leaving a board screen brings it back, so no destination is ever more than the page's own back control away and no capability is withdrawn. *(Owner recommendation from the device pass, 2026-07-31: hide it on replay to give the move list more room — offered as optional, "keep as is is also ok" — then extended to the play screen as well, "to solve the space problem".)*

**The condition is the presentation and not the device**, which is [Layout shapes](#layout-shapes)' own reasoning applied to the chrome instead of the board. What the recommendation is about is a bar standing at the foot of the screen, directly under the play controls and the replay transport, and costing the page its full height; that is the **compact** presentation — an iPhone, and equally a narrow iPadOS window, which gets the same answer for the same reason. Where the container presents any other way it is left alone, because there the reclaimed space is not there to reclaim: an iPad at full width carries its destinations inside the navigation bar rather than beneath the controls, and hiding them was measured to return **nothing at all** on the board and a single collapsed title row on replay, while taking away a visible way to the other destinations. **A Mac's sidebar is not a tab bar and this rule never reaches macOS**, where nothing about the window changes.
...
- **Human versus AI** — **悔棋**, **判和**, **认输**, and **翻转棋盘**. *(Owner recommendation, 2026-07-31, recorded in [issue #80](https://github.com/ppppvz/MiniXiangqi/issues/80).)* This mode had no flip control, on the reasoning that the accepted orientation behaviour already places the human's own side at the bottom. That reasoning was never that the control does harm, which is what the owner's recommendation turns on: somebody wanting to read the position from the other side has the same reason to in either mode, and the orientation the mode chooses for them is where the board starts rather than where it has to stay. The control is the same presentation-only flip Free Play and replay carry — the same key, the same word — so nothing but its placement is new. **It takes no tint**, and the cluster's one tinted moment is still the concluding action.
...
**The replay cluster is a grid.** Its five transport controls are one row of equal cells with one gap between them, and **翻转棋盘** beneath spans the width the row spans, so the cluster's leading and trailing edges are the header's and the move list's. Sizing each control by its own glyph drew five different widths for five equal steps; a second, smaller gap inside the row than between the rows made the pair read as two clusters dropped beside each other rather than one laid out; and with neither row's trailing edge anywhere in particular the cluster had a left edge and no right one. One gap, five equal cells, one width. *(Owner feedback from the iPhone, 2026-07-30: the buttons work and sit in reasonable places — what wanted fixing was their positional relationships to each other and their alignment.)*

**Where the fourth control goes belongs to the layout rather than to this list**, and it is **the cluster's last place on every platform** — the one Free Play's cluster already carries it in. *(Settled against rendered screens, 2026-07-31, on the owner's recommendation above.)* What differs between the platforms is only what the panel does when the cluster's own row will not hold four sets of words, and the answer is the arrangement each platform's replay cluster already uses in that same panel.

- **Stacked.** The cluster carries it, in one row. This was the placement genuinely in doubt — the judgement above bites here, and the toolbar beside **棋谱** was the standing alternative — and a 402-point phone answered it: at that width the row holds four controls with room to spare in Chinese, and holds them in English with **翻转棋盘** at its symbol, which is the fallback the cluster already had. A control that fits where the cluster is does not need a second home.
...
- **The room that move frees is shared, and the share leans to the board.** Moving the pair out of the panel does not make the page any shorter, so what the move list gains beneath the board the board pays for above it, point for point. Two things keep that price small: beneath the board the panel asks for **less** than it does beside the board, where nothing is competing with it; and the pair claims **no air below itself** there, because the board's own allowance is already that air. What is left is a cost rather than a bargain, and it is stated rather than hidden: on a 402-point iPhone the pitch goes from 47 to 46 and on an iPad in portrait from 98 to 97, while the move list goes from three rows and a sliver of a fourth to four rows read whole on both. The side-by-side shape pays nothing, because there the pair was always the top of the panel. Where the space is shorter still it is the panel that yields, not the board — the grant under [Layout shapes](#layout-shapes) reserves the board's floor first, and replay could stand on that floor if it had to, its board being a read-only document with no touch target on it. *(Owner feedback from the iPhone, 2026-07-30: the result line moves to the top of the page and the freed bottom space goes to the move list. Owner decision on the pictures, 2026-07-31: share that space, leaning to the board, rather than giving all of it to the list.)*
- **Where the destination bar stands across the bottom, this screen hides it**, which is the general rule in [Navigation](#navigation) and this is the screen it was first asked for. What the reclaimed height actually buys is measured rather than assumed, and on a 402-point iPhone it is the **board**: the pitch goes from 46 to 50, which is the largest the phone's width allows, and the move list keeps the four whole rows the round above gave it. The list gains no fifth row because there is none left to gain — the board stops at the width and the remainder is air — so this is the shared, board-leaning split of the round above arriving at its end rather than a second decision. *(Owner recommendation, 2026-07-31.)*
...
*(Windows, 2026-07-31, from the owner's tour, which found a board page with no board on it and nothing said. The mechanism was not this refusal and not the navigation pane: the Windows floor was computed from the content while the platform applies it to the whole window frame, which put the board host under its block at the hard minimum, and the frontend's own view then failed to take a size because the fallback geometry equalled the one it was built with. Both are implementation faults and are fixed as such. What belongs in this contract is the rule they exposed — that no floor is the last word, so the state below one has to speak.)*
...
- Define the haptic events outside board play. Board play's haptic set is defined under [Sound and haptics](#sound-and-haptics), and each half now follows its own accepted preference wherever it would fire. Where and how the two toggles are presented is settled there too, on all three platforms: Windows answered at Stage 6 and offers 声音 alone, **closed as not for the MVP rather than as impossible** — the platform's touchpad haptics API exists and is experimental, 24H2-gated and rarely carried, and the row returns on that evidence under iOS's own ask-the-device rule.
