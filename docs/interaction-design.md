...
- **Style decoration stays inside the disc; markers stay outside it.** On an occupied point, no game-state marker's ink touches the disc face, measured against the disc at its largest — a selected disc is scaled, and the rule holds at that size too. The air gap this leaves is what keeps a marker legible against Traditional's heavier Black ring, Modern's white inset ring, and High Contrast's outlined disc alike, and it is why the choice of characters or icons never affects a marker. Three markers are outside the rule because no disc is present to clear: the legal-destination dot and the drag-origin marker, which belong to empty points, and the pointer hover fill, which is drawn beneath the pieces.
...
**Words and symbols degrade together.** Where the cluster's arrangement holds every control with its word, every control carries it; where it cannot, **every control gives up its word at once** and stands as its symbol, with the word as its accessibility label. No row mixes a worded control with a symbol-only one. An arrangement may still wrap — **Flip Board** drops to a second row where a panel's width will not hold one, as replay's cluster already does — but each arrangement is uniformly worded or uniformly symbolled. The arrangement is the layout's answer and not a promise: what is promised is that every control of the scene is on screen and inside the surface carrying it — the panel's own inset edge side by side, the screen's width stacked — at the smallest size each platform allows. The symbols, the uniform fallback and the drawn scene transition are accepted for the Apple frontend; Windows keeps its column arrangement of these compositions — without Hint, which awaits that platform's own hint work — and adopting this grammar there is recorded register work.
...
- The element carries nothing of the hint: the control, its activity, and its disabling live in the play-control cluster, defined in full under [Hints](#hints).
...
- **That pair is the top of the page in both layout shapes.** Beside the board it is the panel's first section, which is already the top. Beneath the board it is **above** the board rather than under it — where the stacked arrangement puts what is true of the game, and where the play screen puts its own turn status. The screen is therefore one design in both shapes: what the game was, then the board, then the record and the transport.
...
- **Where the destination bar stands across the bottom, this screen hides it**, which is the general rule in [Navigation](#navigation). What the reclaimed height buys is the **board**: on a 402-point iPhone Mini Xiangqi reaches pitch 57 and Xiangqi 44 under the full-width fitting above, each the largest its width allows, while the move list keeps its usable rows.
...
**A stacked board screen is fitted to the full width.** The shape spends no horizontal allowance on a board screen: the block takes the screen's width, the whole-point pitch leaves a few points of remainder at most, and the style's board surface runs to the screen edges beneath the numeral strips, so the board meets the glass without a sliver of page beside it. On an iPhone that is what the shape exists to buy — a 393-point screen carries Xiangqi at pitch 43 rather than 38 — while an iPad in portrait is bound by the maximum footprint below instead, exactly as before. The air the allowance held moves above and below the board, where the height is. A pre-start preview keeps its air: it sits among setup controls on a page rather than alone on a screen, and the full-width rule is the board screens'.

At their floors, Mini Xiangqi's core is 308 points square and Xiangqi's is 306 by 340. On a 402-point iPhone, width fitting gives Xiangqi a pitch of 44 points rather than forcing the board off screen. The smallest supported Mac display and largest-text setting remain running-frontend gates for the board, chrome and longer metadata together.
