...
- **The room is every device that can be reached** — a device this one is paired with, and a device on the same local network running the application. Each is one row, one device, in one list; a device that is both is one row and not two; and **no row says which of the two it is**, the player choosing somebody to play rather than a way of reaching them. **Nothing in this feature ever names a transport, a band, or a path** — not a row, not a status line, not a refusal, not a permission.
- **A device is in the room while its own player has nearby play awake.** A paired device is listed from the system's pairing record, whether or not a connection stands to it. A device that is not paired is listed while it is on the network with its player in the nearby feature or holding a live game; nothing is reachable merely because the application is installed, and the two are bracketed alike. **The invitation is available where the chosen device holds a live connection**, and waits where it does not.
...
- **The permission the local network needs is the system's own, asked in context.** The first use of nearby play raises it and nothing before that does, over the surface the player has just opened, with a plain sentence saying what it is for. Declined, nearby play is exactly what it is without it — the same rows, the same sheet, the same board, a shorter reach — and the app neither asks again nor explains nor refuses anything on its account: a device that cannot be reached is not in the room, which is what an empty room already means.
