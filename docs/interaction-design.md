...
- **A shown hint.** No marker of its own, and no new geometry: the suggested piece is taken up exactly as the player's own tap takes one up — the selection ring, the lift, and every legal destination — and the suggested destination's own marker, the dot or the capture ring it already has, is strengthened by exactly the amount the illegal-tap answer strengthens the whole set by, so it stays inside the same cell and the same limits. It arrives with one swell and then holds, because what the strengthening says is which destination the engine chose; under Reduce Motion the strengthened state arrives without the swell. See [Hints](#hints).
...
- **Human versus AI** — **Undo**, **Claim Draw**, **Resign**, and **Flip Board**. The flip is the same presentation-only control Free Play and replay carry, under the same key and the same word. **It takes no tint**, and the cluster's one tinted moment is the concluding action.
- **Free Play** — **Undo**, **Claim Draw**, **Flip Board**. It cannot resign, having no opponent to resign to.
- **Nearby** — the acts of whoever's turn it is, then **Resign** and **Flip Board**, per [Nearby play](#nearby-play). Never more than four, which is the composition this list's fourth-control rule already answers for.
...
**Where the fourth control goes belongs to the layout rather than to this list**, and it is **the cluster's last place on every platform** — the one Free Play's cluster already carries it in. What differs between the platforms is only what the panel does when the cluster's own row will not hold four sets of words, and the answer is the arrangement each platform's replay cluster already uses in that same panel.

- **Stacked.** The cluster carries it, in one row. At 402 points the row holds four controls with room to spare in Chinese, and holds them in English with **Flip Board** at its symbol, which is the fallback the cluster already had.
- **Side by side.** The panel is 260 points wide on both platforms and the cluster has 228 inside its inset, but the two frontends spend that differently. Windows stacks its play controls in a column of their own width, so a fourth costs height and the panel simply carries it. The Apple frontend lays them in a row, so a fourth costs width — and four English labels come to 359 points, more than the row has even with the trailing word given up. There **Flip Board drops to a second row beneath the others**, which is what the Apple replay cluster already does with this same control in this same panel, and it keeps its word where the row above had no room for it. Chinese fits the row at the symbol and stays in it.

The arrangement is the layout's answer and not a promise. What is promised is that all four controls are on screen and inside the surface carrying them — the panel's own inset edge side by side, the screen's width stacked — at the smallest size each platform allows.

While a game is finished there is no draw left to judge, so that slot carries the concluding action instead, and **Resign** goes with the game it could have ended: the finished cluster is **Undo**, the concluding action, and **Flip Board**, in both modes, for as long as the game is finished.
...
- **On the player's own turn the element carries the hint control**, which is the same slot's counterpart on the other turn. It is defined in full under [Hints](#hints).
...
- **The control is in the turn status**, beside the **You** controller label in human-versus-AI play and beside the side-to-move line in Free Play, which has no controller label. It is the AI activity slot's own placement grammar on the other turn, and it takes the weight the platform gives an action written into a line of prose — the stalled state's inline **Try Again** is the precedent. The play-control cluster stays at its accepted compositions; a hint is not one of them.
- **It is present exactly when a hint is possible, and absent otherwise** — absent rather than disabled, since a word standing greyed out through the machine's turn is a promise about a turn that is not the player's. Possible means: human-versus-AI or Free Play, the game live, and the turn the player's own — the human's against the AI, either side's in Free Play. Never in [nearby play](#nearby-play).
- **The presentation is the board's own language**, defined under [Game-state markers](#game-state-markers): the suggested piece is taken up exactly as a tap takes one up, and the suggested destination's marker is strengthened. Tapping that destination commits the move through the ordinary input path, and every ordinary selection interaction applies unchanged — switching to another piece, tapping the held piece again, tapping outside the board.
- **Activity is shown only past the threshold the AI's own activity waits out**, and in the same form: a small system activity indicator beside the control, carrying no material.
...
- **Where the destination bar stands across the bottom, this screen hides it**, which is the general rule in [Navigation](#navigation). What the reclaimed height buys is the **board**: on a 402-point iPhone Mini Xiangqi reaches pitch 50 and Xiangqi about 39, each the largest its width allows, while the move list keeps its usable rows.
...
At their floors, Mini Xiangqi's core is 308 points square and Xiangqi's is 306 by 340. On a 402-point iPhone, width fitting gives Xiangqi a pitch of about 39 points rather than forcing the board off screen. The smallest supported Mac display and largest-text setting remain running-frontend gates for the board, chrome and longer metadata together.
