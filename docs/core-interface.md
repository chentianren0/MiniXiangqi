...
- `MxqEndReason` — the fixture reason identifiers `CHECKMATE`, `STALEMATE`, `THREEFOLD_REPETITION`, `PERPETUAL_CHECK`, `PERPETUAL_CHASE`, the mutual-violation reasons `MUTUAL_PERPETUAL_CHECK`, `MUTUAL_PERPETUAL_CHASE`, the move-count draw `FIFTY_MOVE_RULE`, and the user-scoped `RESIGNATION`, `ENDED_EARLY`, plus `NONE`. `FIFTY_MOVE_RULE` is Xiangqi's alone; Mini Xiangqi has no move-count rule, so it can never arise there.
- `MxqGameConfig` — the game's frozen configuration, and `game` is part of it: the play mode, the resolved human side, the AI level and its exact frozen movetime, the first-mover choice, and which of the two games is being played. Every later question about that session is asked under it.
...
`MxqArchiveInfo` carries format version, the game the file's `rules_id` names, the stable game identity, timestamps, mode, human side, `MxqOutcome`, `MxqEndReason`, and move count.
