...
- `MxqSetupViolation` — why a position is not one to set up in: `rule`, the clause broken, from the closed `MxqSetupRule` vocabulary `PIECE_COUNT`, `PALACE`, `ELEPHANT_SIDE`, `SOLDIER_RANK` and `OPPONENT_IN_CHECK`, which are [xiangqi-rules.md](xiangqi-rules.md)'s setup clauses in the order that document states them; `side`, whose pieces the clause is about; and `square`, the point at fault where the clause names one and empty where it names none. It carries one violation rather than a list — the first clause the position breaks — because what a caller does with it is tell a person composing a position what to fix next.
...
- `MXQ_API_VERSION` (major/minor/patch) is compiled into the header, reported by `mxq_core_version`, and checked at `mxq_core_init`. The header ships at 3.2.0. Within a major version: no removals, renames, signature changes, enum renumbering, struct-field changes, or thread-rule tightening; additions are new functions, appended enum constants, appended struct fields behind `struct_size`, and a reserved field claimed by a real one of its own width — the layout does not move, and a reserved field was never a value a caller could read. **The minor carries exactly those additions**, of which a session-free entry and a configuration member appended behind `struct_size` are two; everything else is a major, a capacity a struct carries by value included, because a caller compiled against the smaller struct declares a size this build refuses and the refusal is the point. The compatibility promise is source-level across the repository's own two frontends, which ship in lockstep.
