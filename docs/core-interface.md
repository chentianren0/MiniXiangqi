...
Because every session is store-attached and every stored row carries the document its game encodes to, a game the archive format has no `rules_id` for cannot have a session: creation returns `MXQ_ERR_ARG_RANGE` without asserting, reporting two parts of one core at different stages of one widening rather than a caller outside a vocabulary it owns. The rules facade answers for such a game in full; persistence alone refuses it.
