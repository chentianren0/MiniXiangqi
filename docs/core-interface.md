...
- `MXQ_API_VERSION` (major/minor/patch) is compiled into the header, reported by `mxq_core_version`, and checked at `mxq_core_init`. The header ships at 3.1.0. The minor moved with `mxq_engine_profile_id`, a new function and nothing else — the smallest addition class this policy names, and the one that leaves every struct's layout and every existing signature where they were. The major moved with the FEN capacity: `MxqPosition` carries its position by value, so widening the capacity changes that struct's size, which is exactly the struct-field change a major version is for. Nothing behind `struct_size` could have absorbed it — a caller compiled against the smaller struct declares a size this build refuses, and the refusal is the point. The two new games rode along; appended enum constants would not have needed a major of their own. The previous major moved with the game axis: four rules functions and `mxq_engine_prepare` gained the game as a parameter, and the two per-game engine-profile fields left `MxqVersion` for `MxqGameProfile`. Within a major version: no removals, renames, signature changes, enum renumbering, struct-field changes, or thread-rule tightening; additions are new functions, appended enum constants, appended struct fields behind `struct_size`, and a reserved field claimed by a real one of its own width — the layout does not move, and a reserved field was never a value a caller could read. The compatibility promise is source-level across the repository's own two frontends, which ship in lockstep.
...
- **The generated declarations are `[DllImport]`, not `[LibraryImport]`.** ClangSharp has no `LibraryImport` mode — through 21.1.8.4 its library-name option is the string to use in the `DllImport` attribute, and it emits `extern` methods, which `[LibraryImport]`'s `static partial` shape cannot be spelled as. Generation is what stops sixty-one signatures, fifteen value structs and every capacity constant from being transcribed by hand. Nothing observable is lost: with runtime marshalling disabled and every parameter blittable, the two attributes produce the same call, and the difference is only whether the stub is generated at compile time or at first call. Hand-written platform P/Invokes in the frontend, which no generator writes, do use `[LibraryImport]`, and there it is load-bearing rather than stylistic: under `DisableRuntimeMarshalling` a `DllImport` cannot honour `SetLastError`, while `LibraryImport`'s generated stub reads the error explicitly. Revisit if ClangSharp gains the mode.
