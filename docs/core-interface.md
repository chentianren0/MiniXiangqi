...
- **`mxq_search_start_hint` is that entry asked for the other reason**: the move to show the player, for the side to move in the session's current position, advisory and never applied by the core. It is `mxq_search_start` in every other respect — the same snapshot, ticket space, cancellation, `(game_id, position_revision)` staleness identity, rejection ladder with its legality check, retention under the ticket, callback on the engine thread, and `MXQ_ERR_STATE_ENGINE_NOT_READY` for a session of a game the engine is not prepared for — and differs in exactly three things. Its `movetime_ms` is the thinking time itself rather than a cross-check, and must be one of `MXQ_MOVETIME_FAST_MS`, `MXQ_MOVETIME_STANDARD_MS` and `MXQ_MOVETIME_DEEP_MS`, the only three the product defines; any other value is `MXQ_ERR_ARG_RANGE` and **does** assert, because it is a caller choosing outside a closed vocabulary it owns rather than two components disagreeing. Nothing is compared with the session's frozen `ai_movetime_ms`, which is why the entry exists at all: Free Play freezes none and is owed a hint all the same. It refuses with `MXQ_ERR_STATE_SEARCH_IN_PROGRESS` while any search is outstanding, its own kind included, because the engine thread runs one at a time and a hint queued behind another would be answered for a position the player has left. And it demands a session that could still take the move it proposes — `mxq_game_apply_move`'s own state gate, so a detached replay is `MXQ_ERR_STATE_SESSION_READ_ONLY`, an archived game `MXQ_ERR_STATE_SESSION_ARCHIVED`, and a position with a result of its own `MXQ_ERR_STATE_GAME_OVER`, while a claimable neutral repetition has its hint like any other ongoing position.
