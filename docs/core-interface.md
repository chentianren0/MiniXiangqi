...
- **There is no legal-move capacity constant.** The buffer protocol is the answer: `out` NULL with `cap` 0 asks for the count, and `MXQ_ERR_ARG_BUFFER_TOO_SMALL` carries `required_size`, so a caller can always size correctly in two calls and never has to guess. For a binding that would rather use one fixed array, the bound is derived per game, because the board is what it is derived from.
  ...
  **Jieqi: 119**, which is Xiangqi's derivation on Xiangqi's board and the same number. Freeing the advisor and the elephant does not raise it — each reaches four points wherever it stands, which is what the derivation already allowed each — and a face-down piece moves as the piece whose square it stands on, so it can only offer what that piece offers. No measured position is pinned for it either.
