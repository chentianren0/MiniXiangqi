...
- **The one documented exception to "off the UI thread": the active game's own calls.** The store-attached active game's lifecycle and commits — `mxq_game_create`, `mxq_game_resume_active`, `mxq_game_apply_move`, `mxq_game_undo`, and the terminal commits — may be driven synchronously from the UI thread. The per-move commit measured median ~2 ms, worst ~2.2 ms on Mac hardware (the measurement is on issue #50's record), and the owner's proportionality ruling of 2026-07-29 is that one such commit per human move does not justify restructuring the input gate around asynchrony: rigor scales to consequence, and invisible-in-a-frame performance does not drive architecture. The exception is exactly this call family on exactly this session; every other row above stands, the off-main-actor structure is held in reserve, and the number is re-measured on device at Stage 6 before a phone inherits the exception.
