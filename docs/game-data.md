...
- Three `STRICT` tables: `meta` (non-authoritative bookkeeping; nothing reads it to decide anything), `game` (one row per stored game), and `library` (exactly one row, enforced by constraint and a permanence trigger, holding the single nullable active-game reference). There is one active game across both games and every mode, not one per kind.
