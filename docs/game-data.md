...
A refusal is the whole of what happens. A store this build cannot read is left exactly as it was found — not migrated, not renamed, not emptied — so the app has no library until that file is removed by hand, and the publish page states that path. Discarding a user's games silently to make the app start would be the one outcome worse than not starting.
