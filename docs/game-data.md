...
- **A `jieqi` document's deal rides in `start_fen`, and the format adds nothing else for it.** The dealt start holds every hidden identity, so the deal and the move line are the whole record: each position, each reveal and each disclosed capture replays from them, and a per-move reveal field would be a second copy of what the replay already decides. Nothing of how two devices came to hold that deal is content either, by the portability law.
...
A dealt game is no exception. The two devices derive one deal from the handshake that protocol defines, each writes it into its own record's `start_fen`, and none of what the handshake was made of — the commitment, the nonce, the seed, the digest — is content: it is the wire session's own bookkeeping, which the portability law keeps out of every archive. So two devices' records of one dealt game carry the same start and the same plies, and differ exactly where any two records of one nearby game differ.
...
- **The four deal values are present exactly for a `jieqi` session and absent for every other**, which is the same pairing the protocol draws between a hidden-information game and the handshake its session opens with. They are what a resumed session re-verifies its own deal against before using it and what it compares with the other device, per that protocol; the deal is derived from them rather than kept here, and where the deal is recorded is the game row's own `start_fen`.
