...
A dealt game is no exception, and the portability law decides its parts one at a time. The two devices derive one deal from the handshake that protocol defines and each writes it into its own record's `start_fen`; the commitment, the nonce and the seed it came from stand in content beside it, because they are facts about the game rather than about either device — both ends hold the same three, and they are what lets anybody check the deal was dealt rather than chosen. Everything else the session held is the wire's own bookkeeping and stays out, the digest with it, being derivable. So two devices' records of one dealt game carry the same start, the same three values and the same plies, and differ exactly where any two records of one nearby game differ.
