...
Device-local facts therefore never enter archive content: the identity the transport names a peer by, which side of a networked game this device's player took, pairing or device names, connection history, and the wire session's own bookkeeping. Local perspective is store metadata, beside pin state and provenance, and never travels in an exported file.

A networked game is played by two devices, and each keeps its own record of it. Identity is minted locally at creation and the instants are each device's own clock, so the two records are two records.
...
- Canonical form: UTF-8, one line, members in codepoint order, no insignificant whitespace, integers only, `null` forbidden, minimal string escaping. The core is the only writer, and the canonical bytes are what content equality hashes. Some of these clauses decide what a document *means* — UTF-8, integers only, and `null` forbidden — and reading enforces them; the rest describe the writer's output and are re-established by the canonicalization stage of the validation order below rather than demanded of an incoming file, so a file spelling an equivalent document differently is canonicalized rather than refused.
- No move count is serialized (it is the length of `moves`); no engine, fork, or NNUE identifier is serialized — a game's meaning derives from the rules contract (`rules_id` + `rules_version`), never from the engine build that played it. `ai_movetime_ms` is stored beside `ai_level` so a later retuning of a level's time does not reinterpret existing archives; import checks both for presence and range, not the current pairing. `first_mover_choice` is retained because it cannot be reconstructed later. What `rules_version` means for a game is owned by the contract that owns that game's rules — [xiangqi-rules.md](xiangqi-rules.md) for Mini Xiangqi and Xiangqi, [jieqi-rules.md](jieqi-rules.md) for Jieqi, and [placement-engine-integration.md](placement-engine-integration.md) for the placement games, whose rules authority is their pinned engine. It is one recorded interpretation version, `1` for every game this build carries: a value that moved for one game and not another is a distinction this format does not draw.
...
- `mode`: `human-vs-ai`, `free-play`, `nearby`, `online`. `nearby` and `online` are the **networked** modes — one game played by two devices over the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md) — and every rule below that turns on there being a player on the other end names networked play rather than either of them: how the two devices reached each other is a fact about the transport, and this format records the game. `human_side`: `red`, `black` (the resolved side). `ai_level`: `fast`, `standard`, `deep`. `first_mover_choice`: `human-first`, `ai-first`, `random`.
...
- `rules_id`: `minixiangqi`, `xiangqi`, `jieqi`, `gomoku-15`, `renju`. Which of them a build carries is the build's: a build without the engine a game is played on cannot read that game's board, and a document naming it meets the same closed-vocabulary refusal any other unknown `rules_id` meets.
...
- Cross-field rules, enforced at import and as store constraints:
  ...
  - the deal members exactly when `rules_id = jieqi` and `mode` is a networked one, and never otherwise: a locally dealt game has no handshake behind it to record, and no other game has a deal;
  - a rule reason belongs to the kind of game whose rules produce it, in both directions: the movement reasons only with `rules_id` naming a movement game, and `five-in-a-row` and `board-full` only with one naming a placement game. The declared ends belong to every game and are governed by mode instead;
  - `fifty-move-rule` only with `rules_id = xiangqi` and `forty-move-rule` only with `rules_id = jieqi`, both narrower than the rule above: Mini Xiangqi has no move-count rule, and the games that do count are counting to different numbers.
...
A `nearby` or `online` game is one game played by two devices over the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md). The mode records how the two devices reached each other — where they are, or at any distance — and decides nothing else here: the networked modes are one kind of record, and every clause below holds for each. Such a game's archive records the same things every archive records, and its ending is that protocol's own:
...
A networked game omits the `human-vs-ai` configuration members, as `free-play` does. Nothing in content names a device or a player, the transport that carried the game included.

A dealt game is no exception, and the portability law decides its parts one at a time. The two devices derive one deal from the handshake that protocol defines and each writes it into its own record's `start_fen`; the commitment, the nonce and the seed it came from stand in content beside it, because they are facts about the game rather than about either device — both ends hold the same values, and they are what lets anybody re-derive the deal and check the record against itself. Everything else the session held is the wire's own bookkeeping and stays out, the digest with it, being derivable. So two devices' records of one dealt game carry the same start, the same evidence and the same plies, and differ exactly where any two records of one networked game differ.
...
- A networked game has no unilateral undo: a retraction there is the two players' agreement, and what it retracts is what they agreed to keep.
...
- Every table is `STRICT`: `meta` (non-authoritative bookkeeping; nothing reads it to decide anything), `game` (one row per stored game), `library` (exactly one row, enforced by constraint and a permanence trigger, holding the single nullable active-game reference), and `nearby_session` (below). There is one active game across every game and every mode, not one per kind.
...
- `provenance`, `pinned`, the History-added time, and `local_side` are local library metadata rather than derived summary, and are the only columns the blob does not decide. `local_side` is the side this device's player took, present exactly for a locally played networked record and absent everywhere else — an imported networked record has no local player, and no local mode has two. It never enters the archive and never travels in an export.
...
`nearby_session` holds what the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md) needs to continue an interrupted session after the application has been relaunched. It is the store's one table for a session rather than a game, and every value but the commitment, the nonce and the seed is device-local by the portability law, which is why they are in no archive: the identifier the proposer minted, the peer identity on the other end, which peer proposed, the accepted-retraction count and the ply count the last retraction survived to, the terminal this device has sent if it has sent one, whether the last ply is the `claim` turn action — the one ply the archive deliberately does not record, and therefore the one an unfinished game has nowhere else to carry — and, for a dealt game, the values the session's deal came from: the commitment, the nonce, the seed, and the deal digest.

- **The deal values are present exactly for a `jieqi` session and absent for every other**, which is the same pairing the protocol draws between a hidden-information game and the handshake its session opens with. A resumed session re-verifies its own deal against all of them before using it, and the digest is the one a `resume` carries, so it is the only one ever compared with the other device. The deal itself is derived from them rather than kept here, and where the deal is recorded is the game row's own `start_fen`. All but the digest reach the archive as well, per the archive shape above — they are the game's own evidence rather than this device's — and they stand here too because a session must re-verify its deal without decoding a document that does not exist until the game is filed.
- It is a table of its own, not columns on `game`. The two have different lifetimes and change on different beats: a terminal this device sent moves the session and not the game, and a networked game is a small minority of the rows a library holds. One table serves every networked mode: what the protocol needs to continue an interrupted game is the same whichever way the two devices reached each other, so the transport is nowhere in these columns.
...
- A row belongs to a locally played, unfinished networked game, by trigger — which is the same pairing that makes `local_side` present.
- Nothing expires it: the protocol's settlement and retirement, and the player's own choice, are what a stale session meets. A networked game whose session the protocol has parted with is filed as it stands rather than left in the library as an active game nothing can play.

## Settings placement

The persistent preferences accepted in [product.md](product.md) — the default first-mover choice, the default AI level, **Confirm Before Deleting**, **Confirm Before Placing**, the sound and haptics toggles, the piece symbols, and the notation — live in **each platform's own preference system**, not in the shared store.

- The core's ownership rule is that what is correctness-critical exists exactly once. No preference is: the ones that affect a game are read only when one is created; **Confirm Before Deleting** gates a permanent deletion but does not perform it, and the core's own deletion invariants hold whatever the preference says; and the rest are presentation or device capability.
