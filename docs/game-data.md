...
- `mode`: `human-vs-ai`, `free-play`, `nearby`, `online`. The last two are the **networked** modes — one game played by two devices over the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md) — and every rule below that turns on there being a player on the other end names networked play rather than either of them: how the two devices reached each other is a fact about the transport, and this format records the game. `human_side`: `red`, `black` (the resolved side). `ai_level`: `fast`, `standard`, `deep`. `first_mover_choice`: `human-first`, `ai-first`, `random`.
...
- Cross-field rules, enforced at import and as store constraints:
  ...
  - `outcome = draw` exactly when the reason is one of the eight draw reasons — the six rule draws, `agreed-draw`, and `mutual-resignation`;
  ...
  - `human-vs-ai` never with `rules_id = jieqi`, and with it none of that mode's four configuration members: the game has no AI, so a document recording one against a machine records a game this app cannot have played;
  - the three deal members exactly when `rules_id = jieqi` and `mode` is a networked one, and never otherwise: a locally dealt game has no handshake behind it to record, and no other game has a deal;
  - a rule reason belongs to the kind of game whose rules produce it, in both directions: the nine movement reasons only with `rules_id` naming a movement game, and `five-in-a-row` and `board-full` only with one naming a placement game. The four declared ends belong to every game and are governed by mode instead;
  - `fifty-move-rule` only with `rules_id = xiangqi` and `forty-move-rule` only with `rules_id = jieqi`, both narrower than the rule above: Mini Xiangqi has no move-count rule, and the two that do count are counting to different numbers.
...
A networked game omits the four `human-vs-ai` configuration members, as `free-play` does. Nothing in content names a device or a player, the transport that carried the game included.

A dealt game is no exception, and the portability law decides its parts one at a time. The two devices derive one deal from the handshake that protocol defines and each writes it into its own record's `start_fen`; the commitment, the nonce and the seed it came from stand in content beside it, because they are facts about the game rather than about either device — both ends hold the same three, and they are what lets anybody re-derive the deal and check the record against itself. Everything else the session held is the wire's own bookkeeping and stays out, the digest with it, being derivable. So two devices' records of one dealt game carry the same start, the same three values and the same plies, and differ exactly where any two records of one networked game differ.
...
`nearby_session` holds what the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md) needs to continue an interrupted session after the application has been relaunched. It is the store's one table for a session rather than a game, and all but three of its values are device-local by the portability law, which is why they are in no archive: the identifier the proposer minted, the peer identity on the other end, which peer proposed, the accepted-retraction count and the ply count the last retraction survived to, the terminal this device has sent if it has sent one, whether the last ply is the `claim` turn action — the one ply the archive deliberately does not record, and therefore the one an unfinished game has nowhere else to carry — and, for a dealt game, the four values the session's deal came from: the commitment, the nonce, the seed, and the deal digest.

- **The four deal values are present exactly for a `jieqi` session and absent for every other**, which is the same pairing the protocol draws between a hidden-information game and the handshake its session opens with. A resumed session re-verifies its own deal against all four before using it, and the digest is the one of them a `resume` carries, so it is the only one ever compared with the other device. The deal itself is derived from them rather than kept here, and where the deal is recorded is the game row's own `start_fen`. Three of the four reach the archive as well, per the archive shape above — they are the game's own evidence rather than this device's — and they stand here too because a session must re-verify its deal without decoding a document that does not exist until the game is filed.
- It is a table of its own, not columns on `game`. The two have different lifetimes and change on different beats: a terminal this device sent moves the session and not the game, and a networked game is a small minority of the rows a library holds. One table serves both networked modes: what the protocol needs to continue an interrupted game is the same whichever way the two devices reached each other, so the transport is nowhere in these columns.
