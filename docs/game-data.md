...
- Cross-field rules, enforced at import and as store constraints:
  ...
  - the three deal members exactly when `rules_id = jieqi` and `mode = nearby`, and never otherwise: a locally dealt game has no handshake behind it to record, and no other game has a deal;
...
A `nearby` game is one game played by two devices over the protocol in [boardgame-protocol-v2.md](boardgame-protocol-v2.md). Its archive records the same things every archive records, and its ending is that protocol's own:
