...
- **`MXQ_NNUE_SOURCE` is an override rather than the way a build finds the network.** Its default is the committed file. Pointing it elsewhere is how a candidate network is tried before it is committed; replacing the network for good is the bytes and the manifest, and nothing else.
