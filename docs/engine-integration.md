...
**The Apple app bundles the assets as one pack rather than as the files themselves.** `apple/build-core-xcframework.sh` verifies every input against its pins and then writes `engine-assets.mxqpack` into the app's resources; the app stages the directory the core reads from it into its own caches at start-up, and the core's verification of each staged file is unchanged. The pack exists because three of the five networks are published files that other applications embedding the same engines ship byte-for-byte, and App Review compares binaries across developers: each entry is compressed where that helps and passed through a keyed transform where it does not, so no run of the published bytes appears in the bundle. `apple/MiniXiangqi/Core/AssetPack.swift` is the format's one implementation, shared by the app and the packer the generator compiles from it.
