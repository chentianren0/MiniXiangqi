...
**iOS and iPadOS keep the rule above instead**, and the distinction is the platform's rather than a preference: there the memory warning is the last signal before the system reclaims the process, so it is a suspension in everything but name, and answering it by re-allocating gigabytes is the one response certain to make the outcome worse. What replaces the desktop's immediate re-ask on that platform — a wait, a smaller retry trigger, or simply the next thing the player does — is Stage 6's to settle with a device in hand, and is listed below.
...
- Decide what an iOS or iPadOS memory warning does about a search that is owed, once the desktop rule above cannot be borrowed: there the warning precedes reclamation, so the release is a release and the immediate re-ask is not available. Stage 6 settles it with a device in hand.
