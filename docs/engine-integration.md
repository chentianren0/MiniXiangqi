...
**Two of the three desktop triggers stand on Windows and one does not.** App termination is handled: closing the window performs the core's own shutdown once, which cancels every search, joins the engine thread and closes the store in one call, exactly as the ordering below requires. Memory pressure is the clause above. **A system-sleep notification is not observed yet, and that is owed rather than decided** — unlike the pressure signal, Windows defines this one precisely (`WM_POWERBROADCAST` with `PBT_APMSUSPEND`), so the question is only that receiving it needs a window handle and a message hook, which is the part of the frontend no headless run can exercise. It is recorded here so that nothing reads the clause above as claiming the Windows frontend implements the whole rule.
...
- Wire the Windows system-sleep notification, which the backgrounding clause above records as owed. It is the one desktop trigger Windows defines precisely and this frontend does not yet receive; it needs a window handle and a message hook, so it belongs with work that has a window rather than with the headless core.
