...
**iOS and iPadOS take the same rule, and the fear behind holding it back turns out not to apply.** This document said the desktop's immediate re-ask was unavailable there — the warning precedes reclamation, and re-allocating gigabytes in reply is the one response certain to make the outcome worse — and left what should replace it to Stage 6. What Stage 6 found is that the re-ask never re-allocates gigabytes: it is the *accepted budget arithmetic* asked again from a **fresh probe**, and on iOS that probe is `os_proc_available_memory()`, which under real pressure is exactly the number that has just collapsed. So the re-ask either sizes the Hash down to what is genuinely left or produces the accepted insufficient-memory presentation. The response the rule was written to prevent is the one the rule already prevents.

So the platforms share one memory-pressure path, which is also what the shared machinery already does: the release is whole, the question of whether a search is owed follows it, and every retry re-probes. Nothing iOS-specific is added. **The owner deferred anything beyond this** (2026-07-30, recorded in issue #80): a wait, a smaller retry trigger, or a pressure design of the platform's own waits until after the MVP, and the accepted worst case — the platform killing the app — loses nothing, because autosave has already committed and resume-at-launch stands.
...
- Revisit iOS and iPadOS memory pressure after the MVP, if the shared path proves not to be enough. It is what ships, by the owner's deferral above; what would reopen it is evidence from a device, not a preference.
