...
- **Correction of adjudication defects that produce false terminal losses** — a chaser counted as chasing while pinned, and a flying-general pin test that counts only the victim's own pieces on the shared file, so a chaser standing between the two generals is invisible to it and a demonstrably free piece is marked pinned. Both are wrong-result defects rather than judgement calls.
- **Completion of the chase-target exemption in the discovered-check classifier path**, so the accepted exclusion of generals and soldiers holds in every path rather than in all but one.
...
- **A 64 MiB reservation for every app-supported search-thread creation path.** The explicit 8 MiB ceiling above still cannot contain the deepest `ALLVARS` search: `MAX_MOVES` is 8192, each `ExtMove` is 8 bytes, and the move array held at each active ply can therefore consume about 15.4 MiB by `MAX_PLY` before the rest of the recursive state is counted. The fork now requests 64 MiB through both `pthread_attr_setstacksize` and `_beginthreadex`. The MSVC path passes `STACK_SIZE_PARAM_IS_A_RESERVATION`, so Windows reserves address space and commits pages as the stack reaches them instead of eagerly committing 64 MiB per worker; supported Apple and pthread platforms likewise back the requested stack as it grows. This changes the stack ceiling, not search or adjudication behavior.

Every change must keep the fork's own suite passing and every approved fixture passing.

Preserving other variants' adjudication exactly is the preferred bar, but it is not absolute, and these corrections are accepted exceptions: the repetition-window parity correction and the pinned-chaser correction. Both remove defects rather than change rules, and both reach built-in `xiangqi`, because a chasing rule is what they act on and `xiangqi` is the only built-in variant that sets one. Neither is a rules difference a variant property could express — the repetition window is the same rule in every chasing variant — and both are inert for any variant that sets no chasing rule, which is every variant in the fork except `xiangqi`, the variants that inherit from it, and our own.

The adjudication corrections rest on execution-confirmed defects rather than on the definitions of protection, interruption, and discovered or pinned attacks that [xiangqi-rules.md](xiangqi-rules.md) leaves open, and each lands together with the fixture that pins it.

The classifier-path completion is different in kind and carries no fixture: a targeted search found no position where the gap changes an outcome, so it may well be unreachable. It is accepted not because a defect was demonstrated but because the accepted exclusion of generals and soldiers as chase targets should hold in every classifier path rather than in all but one.
...
The hint defined in [interaction-design.md](interaction-design.md) § Hints is a search of its own, through the core's own hint entry in [core-interface.md](core-interface.md) rather than through the AI-reply path: the reply entry cross-checks a request's thinking time against the session's frozen one, and Free Play freezes none.

- **The thinking time is the game's where the game froze one, and the Settings default AI level's where it did not.** A human-versus-AI hint therefore takes exactly the movetime that game froze at creation — a Deep game gives deeper hints, and the hint is never a different opponent from the one playing — and a Free Play hint takes the level the player has made their persistent default. No new preference is added for it.
- **Free Play prepares the engine lazily, at the first hint request**, through the same fresh probe and the same budget arithmetic every other preparation uses, and reports the same failures. A Free Play game that never asks for a hint prepares nothing and costs nothing.
- **Teardown and the suspension path are unchanged in shape and reach further only in scope**: leaving the board releases whatever engine stands, in any mode, rather than only in human-versus-AI play, and the suspension and memory-pressure paths cancel and release exactly as they already do, hint searches included.
- **A hint commits nothing.** Its result is inert like every other search result, is checked by the same rejection ladder and the same `(game_id, position_revision)` identity before it is shown, and becomes a move only through the ordinary apply the player asks for.
- **A hint is cancelled by the player's own commit, before the AI's reply is requested**, as well as by an Undo, leaving the board, suspension, and the game ending. The commit case is a requirement rather than a courtesy: the engine thread runs one search at a time and queues the rest, so a hint still running when the player moves would put the reply behind a whole thinking time of work about a position the game has left.
- **The hint entry refuses while any search is outstanding, and cancellation is asynchronous**, so a request made immediately after a cancellation can be refused for a search that is already stopping. The frontend asks again rather than treating that refusal as a failure.

### Backgrounding and teardown

The trigger is **the platform's own suspension or memory-pressure signal, not loss of focus**. On iOS and iPadOS it is scene backgrounding, and also a foreground memory-pressure warning — the last signal before the system reclaims the process, on the platform where the per-process limit, the memory entitlements and up to 4 GiB of Hash all apply. On macOS, where an unfocused window is still a running app, it is system sleep, app termination, and a memory-pressure notification; switching windows changes nothing. Releasing gigabytes of Hash every time the user clicks another window would be worse than the problem this rule exists to solve.

**On macOS, a memory-pressure notification is not a suspension, and the two are kept apart.** Both take the same cancel-and-release path, whole rather than shrinking Hash in place. But a suspension has a counterpart — the app comes back, and the platform says so — while nothing will ever tell a desktop app that pressure has passed, and the app was never away: it is still in front of the person playing. So a memory-pressure release there is followed immediately by the ordinary question of whether a search is owed, asked again from a fresh probe. Under real pressure that produces the accepted insufficient-memory presentation, which is an answer the player can act on; treated as a suspension it would produce a machine that had quietly stopped playing and would never start again.
...
- **Re-preparation happens when a search is next owed, not on return to the foreground.** A search is owed exactly when the resumed state is an active human-versus-AI game whose committed status reports the AI to move and a search expected. Replay, Free Play, a confirmed result, a game awaiting the user's move, and having no active game all require no engine, so none of them re-prepares one — **except when the player asks for a hint**, which is the one thing that makes a Free Play game, or a game waiting on the player's own move, owe an engine. That preparation happens at the request and never before it.
- Re-preparation obtains a fresh memory probe and can fail, most plausibly because memory conditions changed while the app was away. It reports the same insufficient-memory error as any other preparation. The game remains active, saved, and resumable, with the AI unable to move until preparation succeeds. What the user sees in that mid-game case is settled in [interaction-design.md](interaction-design.md) § Insufficient memory for AI play: the same title, a message that adds the saved-game guarantee, and **Later** / **Try Again** in place of **Cancel** / **Try Again**.
...
- The accepted user-facing levels are:
  - **Fast**: `go movetime 1000`.
  - **Standard**: `go movetime 3000`; this is the new-install default.
  - **Deep**: `go movetime 5000`.
...
- `Threads` is initialized from the active processor count the platform reports at engine initialization — `ProcessInfo.processInfo.activeProcessorCount` — rather than a hard-coded count.
...
- The frontend supplies a fresh available-memory value from a platform memory probe at each calculation. On iOS and iPadOS the probe is `os_proc_available_memory()`, which reflects the process's remaining limit. On macOS, where no comparable per-process limit applies, the probe reports the system's available physical memory.
...
- The memory probe is `os_proc_available_memory()` on iOS and iPadOS; on macOS, `host_statistics64` with `HOST_VM_INFO64`, taking available memory as the free, inactive, and purgeable pages, with physical memory from `sysctlbyname("hw.memsize")`. Each probe's behavior is verified against the accepted budget boundaries on its own platform before that platform ships.
- If allocation fails even though the calculated budget was at least 256 MiB, the AI does not start and the frontend presents the accepted **Couldn't Start the AI Opponent** notice unchanged. The situation the user is in is the same one — memory is not available right now — and the same action resolves it, so a second message would name a distinction they cannot act on differently. Retry re-probes and recalculates exactly as it does for a below-minimum budget, and no smaller Hash is substituted.
...
This engine runs exactly these variants, and the set is closed:
...
- **`EvalFile` is a list, not a path.** The engine accepts several networks separated by a platform separator and selects the first whose basename matches the selected variant, which is how one prepared engine covers every variant without restaging assets. The core hands over every pinned network the asset directory holds, each verified against its own byte length and SHA-256 first, and orders the selected variant's own first so that the token the engine will select is known rather than inferred.
...
- **Effective NNUE off is fatal at preparation, for any variant.** It is never downgraded to a warning and never allowed to reach a search.
...
**One network per variant the core runs, all in this repository**, at `core/assets/`, beside the variant configuration they are loaded with. There is no fetch and no separate location: a build reads them from the checkout.

Each is pinned by exact byte length and SHA-256 in a machine-readable manifest, keyed by the variant it is for, and **nothing stages or loads a byte it has not verified against that variant's own pins** — `core/CMakeLists.txt` for the core's asset staging, the core's engine bridge before the engine is given a path, `build-core-xcframework.sh` for the app's resources. Verifying a network against the other variant's pins is verifying it against nothing, and a mismatch fails the build rather than shipping unverified bytes. That the files are version-controlled does not retire the check: it is what catches a build pointed at other bytes, and damaged weights are invisible without it.

**The Apple app bundles the assets as one pack rather than as the files themselves.** `build-core-xcframework.sh` verifies every input against its pins and then writes `engine-assets.mxqpack` into the app's resources; the app stages the directory the core reads from it into its own caches at start-up, and the core's verification of each staged file is unchanged. The pack exists because three of the five networks are published files that other applications embedding the same engines ship byte-for-byte, and App Review compares binaries across developers: each entry is compressed where that helps and passed through a keyed transform where it does not, so no run of the published bytes appears in the bundle. `MiniXiangqi/Core/AssetPack.swift` is the format's one implementation, shared by the app and the packer the generator compiles from it.
...
- **The Apple app carries what it plays inside the engine asset pack**, `engine-assets.mxqpack`, staged into the app's caches at start-up (see [The networks](#the-networks)). The rule is the same: exactly the pinned set, each verified before an engine sees a path.
...
**How that library is produced is a build decision, not a contract requirement, and it is not the fork's target.** The core compiles the vendored source snapshot through its own CMake, on macOS; nothing in either build consumes an artifact the fork built, because the app needs several architectures out of one build system and the fork's Makefile produces one architecture per invocation. The fork's static-library target exists, as `pinned-inputs.json` records, and satisfies the fork's side of this contract; `core/third_party/fairy-stockfish/README.md` records what would change if the core ever consumed its artifact instead.
