...
A Xiangqi position is one to set up in when all of the following hold. Each of the first four is about where one piece stands, and they are stated first because that is the order a violation is reported in: a position breaking more than one clause is refused under the first, and among the four point clauses under the first offending point, walking the board from `a1` upward.
...
The remaining fifteen are `xq-set-*`, the setup-legality area. Six are one refusal per clause of the predicate above, and three more are the exactness of the point clauses: an advisor refused on a palace edge midpoint, inside the palace and off the five points it reaches, and a soldier refused on its own half for standing off the files it starts on, against the acceptance of the same soldier across the river. Two are the answer for a board still missing a general — the count clause naming the side, and the same board with a misplaced advisor, where the point-anchored violation is what is reported. Three are accepted positions: a composed scene, a side-to-move-in-check position, and the crossed soldier above. The last two are positions of no board at all, a rank spelling eight points and a side-to-move field that is neither `w` nor `b`, which the predicate's own reading of the frozen encoding refuses before any clause is reached. `mx-set-001` and `mx-set-002` are the pair that says Mini Xiangqi defines no predicate — its frozen start accepted, and any other position refused as not that start.
