...
This document is the one contract for legal Mini Xiangqi play and user-visible game results. It owns the adopted interpretation of the rules and the identifiers that connect prose to the executable conformance fixtures. It does not own engine search policy, Fairy-Stockfish implementation details, or UI presentation.
...
The selected public rules source is [PyChess Mini Xiangqi rules](https://www.pychess.org/variants/minixiangqi).
...
Three questions the retained public source does not settle. Each is recorded as an interpretation rather than as a reading of the source, so that an authoritative AXF or CXA text on any of them reopens it as a contract amendment rather than arriving as a discovery.
...
The approved set contains **sixty-six** fixtures. Among them, these pin the rules above in their simplest forms, position identity, and the third-occurrence adjudication point:
