...
The two games share almost everything: one notation, one position record, one adjudication. What differs is the board, the piece set, and the rules that follow from them. Everything below is stated once for both games and marked where it is not.

## The game axis

Every position, move, archive and session belongs to exactly one game. Nothing infers which: these two boards differ in size, so a position of one happens to imply which it is, but the ruleset is not a property of the board — Xiangqi and Jieqi already share one geometry exactly — and a core reading a ruleset off a position would be guessing. The identifier is `rules_id`, and the values below are this document's:
...
Mini Xiangqi has no river, no elephants and no advisors. Xiangqi has them all.
...
A Xiangqi position is one to set up in when all of the following hold. Each of the point clauses is about where one piece stands, and they are stated first because that is the order a violation is reported in: a position breaking more than one clause is refused under the first, and among the point clauses under the first offending point, walking the board from `a1` upward. The count clause follows them because it is about a side and names no point, and the check clause is last.

- **The palace.** Every general stands on one of the nine points of its own palace, `d1`–`f3` for Red and `d8`–`f10` for Black. Every advisor stands on one of the five points of its own palace that an advisor reaches — the four corners and the centre: `d1`, `f1`, `e2`, `d3` and `f3` for Red, and `d8`, `f8`, `e9`, `d10` and `f10` for Black. The general walks the palace and the advisor steps diagonally across it, so the two have the same palace and not the same points.
...
- **The soldiers' points.** No soldier stands behind its own starting soldier rank: a Red soldier on rank 4 or above, a Black soldier on rank 7 or below. And no soldier still on its own side of the river stands off the five files its side's soldiers start on, `a`, `c`, `e`, `g` and `i`, because a soldier that has not crossed moves only forward and so has never left its file. Across the river it moves sideways as well, and every file is one it may stand on.
...
The predicate reads piece placement and the side to move, which is the whole of what a position record denotes. It answers for a position that is still missing a general: that absence is the count clause's answer about that side and not a refusal to answer about the position, which is what lets a surface composing a position be told what is wrong with the first piece put down on an empty board. Whether a position is worth playing is not its question and neither is whether it is already decided: a position offering the side to move no legal move is a legal setup and is not a game to begin, which is [core-interface.md](core-interface.md)'s rule for creating a session rather than this document's.
...
This is one of the rules that differ.
...
- **Soldiers are another difference between the games.** In **Mini Xiangqi** a soldier is excluded exactly as a general is: it may not be a chase target, and its move never creates a chase. In **Xiangqi** a soldier that has crossed the river is an ordinary chase target; one that has not is excluded. The reason is the same in both: the exclusion is of a piece that cannot answer an attack by stepping aside, and a Mini Xiangqi soldier moves sideways from the first move of the game. Whichever way a build reads this, one of `mx-chs-003` and `xq-chs-002` fails it.
...
These questions the retained public source does not settle. Each is recorded as an interpretation rather than as a reading of the source, so that an authoritative AXF or CXA text on any of them reopens it as a contract amendment rather than arriving as a discovery. They all apply to both games.
...
**[jieqi-rules.md](jieqi-rules.md) adopts this document's repetition, perpetual-check and perpetual-chase clauses by reference, and the adoption binds in both directions.** An amendment to those clauses is an amendment to that contract as well, the two games are meant to adjudicate identically, and their `rules_version` move together. Whoever edits them here owes that document the same edit's consequences.
...
The approved set for these two games is what that directory holds under the `mx-` and `xq-` prefixes. Jieqi's own `jq-` fixtures share the directory and are [jieqi-rules.md](jieqi-rules.md)'s inventory, not this document's.
...
For Xiangqi, these pin what the 9-by-10 board adds and what it changes:
...
The rest are `xq-set-*`, the setup-legality area: one refusal per clause of the predicate above; the exactness of the point clauses, an advisor refused on a palace edge midpoint, inside the palace and off the five points it reaches, and a soldier refused on its own half for standing off the files it starts on; the answer for a board still missing a general, the count clause naming the side, and the same board with a misplaced advisor, where the point-anchored violation is what is reported; accepted positions, a composed scene, a side-to-move-in-check position, and the same off-file soldier across the river, which bounds the file clause at the river; and positions of no board at all, a rank spelling eight points and a side-to-move field that is neither `w` nor `b`, which the predicate's own reading of the frozen encoding refuses before any clause is reached. `mx-set-001` and `mx-set-002` are the pair that says Mini Xiangqi defines no predicate — its frozen start accepted, and any other position refused as not that start.
