...
// Which rule produced the value Position::rule_judge() reports. It is purely
// informational and never influences the judgement itself, it exists because the
// value alone collapses a draw repetition, a mutual perpetual check and a mutual
// perpetual chase onto VALUE_DRAW. Which side violated a perpetual is not named
// here, the value already tells it apart.
enum RuleJudgeRule : int8_t {
    RULE_JUDGE_NONE,             // no rule produced a value
    RULE_JUDGE_REPETITION,       // repetition without a perpetual violation
    RULE_JUDGE_PERPETUAL_CHECK,  // repetition decided by perpetual check
    RULE_JUDGE_PERPETUAL_CHASE,  // repetition decided by perpetual chase
    RULE_JUDGE_RULE40            // 40 move rule
};
