...
// Detects chases from state st - d to state st. The optional 'chasing' out-parameter
// reports whether any side was found chasing, which the returned value cannot tell:
// no chase and a mutual chase both score VALUE_DRAW.
Value Position::detect_chases(int d, int ply, bool* chasing) {
...
// The optional 'rule' out-parameter names the rule that produced 'result'. It only reports and never
// judges, and it follows 'result' exactly: it is left RULE_JUDGE_NONE when 'result' is untouched, and
// it is written for the 2 fold value this function reports while still returning false.
bool Position::rule_judge(Value& result, int ply, RuleJudgeRule* rule) {

    if (rule)
        *rule = RULE_JUDGE_NONE;
        ...
        for (int i = 4; i <= end; i += 2)
        {
            ...
            if (stp->key == st->key && (++cnt == 2 || ply > i))
            {
                if (!checkThem && !checkUs)
                {
                    ...
                    // Chasing detection
                    bool chasing = false;
                    result       = rollback.detect_chases(i, ply, &chasing);

                    if (rule)
                        *rule = chasing ? RULE_JUDGE_PERPETUAL_CHASE : RULE_JUDGE_REPETITION;
                }
                else
                {
                    // Checking detection
                    ...
                    if (rule)
                        *rule = RULE_JUDGE_PERPETUAL_CHECK;
                }
