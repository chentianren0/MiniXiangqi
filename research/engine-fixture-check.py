...
FIXTURES = pathlib.Path(__file__).resolve().parent.parent / "MiniXiangqi" / "fixtures" / "rules"
...
def check_fixture(sf, variant, fx):
    ...
    # boundary: one repetition cycle earlier must not yet carry the asserted outcome.
    # A fixture asserting a perpetual loss may legitimately have a *claimable draw* at its
    # boundary prefix (mx-chs-022: the interrupted span is neutral at its third occurrence
    # and only the fourth occurrence is the violation), so only a terminal value is a failure
    # there. A fixture asserting a claimable draw must not have ended at all.
    b = fx.get("boundary")
    if b:
        prefix = moves[: b["prefix_len"]]
        ended_early, value_early = sf.is_optional_game_end(variant, start, prefix)
        draw_asserted = state in ("claimable-draw", "draw")
        if ended_early and (draw_asserted or value_early != 0):
...
def main():
    ...
    fixture_dir = pathlib.Path(sys.argv[2]).resolve() if len(sys.argv) == 3 else FIXTURES
