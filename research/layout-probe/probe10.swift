...
func flush() {
    ...
    try? OUT.write(to: d.appendingPathComponent("probe10.txt"), atomically: true, encoding: .utf8)
}
...
final class SD: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options: UIScene.ConnectionOptions) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            ...
            let widths: [CGFloat] = (640...700).filter { $0 % 4 == 0 }.map { CGFloat($0) } + (1020...1036).map { CGFloat($0) }
            var delay = 0.0
            for cw in widths {
