...
# build-core-xcframework.sh. It cannot be produced during the build:
...
# Under Xcode PROJECT_DIR is the repository root; run by hand there is no
# PROJECT_DIR, and this script's own location is that root.
root=$(cd "${PROJECT_DIR:-$(dirname "$0")}" && pwd)
recorded="$root/Generated/core-inputs.digest"

if [ ! -f "$recorded" ] || [ -z "$(find "$root/Generated" -name libMiniXiangqiCore.a 2>/dev/null | head -1)" ]; then
  echo "error: the shared core has not been built." >&2
  echo "note: run ./build-core-xcframework.sh, then build again." >&2
  exit 1
fi
...
if [ "$(cat "$recorded")" != "$("$root/core-inputs-digest.sh")" ]; then
...
# The engine's assets ship as one pack, MiniXiangqi/Core/AssetPack.swift,
...
recorded_pack="$root/Generated/engine-assets.digest"
if [ ! -f "$pack" ] || [ ! -f "$recorded_pack" ]; then
  echo "error: the engine asset pack has not been staged." >&2
  echo "note: run ./build-core-xcframework.sh, which verifies the assets against pinned-inputs.json and packs them." >&2
  exit 1
fi
...
# And nothing beside it. MiniXiangqi is a file-system-synchronized group,
...
for staged in "$resources"/*; do
  ...
  echo "note: run ./build-core-xcframework.sh, which leaves only the pack there." >&2
  exit 1
done
